...
% Icons are disabled in this book: no icon assets are vendored, so the
% callout header is a clean tinted title bar + colored left rule (no glyph).
...
\begin{tcolorbox}[
  enhanced jigsaw,
  breakable,
  parbox=false,     % let multi-paragraph bodies use the document paragraph spacing
  ...
  title=\hspace{0.4em}\protect#2\hspace{0.5em}{\protect#3}, % no icon: small left pad
