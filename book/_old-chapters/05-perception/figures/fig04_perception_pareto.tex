...
\begin{tikzpicture}[
  ...
  \draw[->, line width=1.2pt, draw=ethdarkblue] (0.8in, -3.8in) -- (5.8in, -3.8in)
    node[midway, below=16pt, font=\scriptsize\bfseries, text=ethdarkblue] {Sense-to-Token Latency ($t_{\text{ingest}} + t_{\text{perceive}}$, ms)};
  ...
  \node[draw=cardborder, fill=cardbg, rounded corners=5pt, line width=0.8pt, text width=2.65in, minimum height=3.0in, inner sep=7pt, align=left] (sidebar) at (7.45in, -2.20in) {
    {\scriptsize\bfseries\color{ethdarkblue} THE 5-VARIABLE PARETO TRADE-OFF}\\[4pt]
    {\tiny\color{ethslate}
