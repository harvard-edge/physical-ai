...
\begin{tikzpicture}[
  ...
  \node[draw=ethdarkblue!30, fill=white, rounded corners=4pt, line width=0.8pt, text width=9.60in, inner sep=6pt, below=0.12in of card_mpu.south west, anchor=north west] (strip_btm) {
    {\footnotesize\textbf{\color{ethdarkblue}The Architectural Invariant:}}\\[2pt]
    {\scriptsize$\bullet$ \textbf{\color{measuredTeal}System 1 (Real-Time MCU):}} {\scriptsize Hard $1000\text{ Hz}$ tick evaluates CBF invariants within $<200\,\mu\text{s}$, completely independent of Linux load.}\\[1pt]
    {\scriptsize$\bullet$ \textbf{\color{consequenceCoral}System 2 (Application MPU):}} {\scriptsize Candidate action chunks stream asynchronously; late, stale, or jittered proposals are safely dropped.}
  };
