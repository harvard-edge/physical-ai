...
5. **See Why the Co-Design Matrix Is Necessary:** Recognize that physical constraints and cognitive obligations have to be reasoned about together rather than in sequence, and locate where the two axes come from. Chapter 2 supplies the columns, Chapter 3 the rows.
...
| :--- | :--- | :--- |
| **MPU Proposes (S2 / S1.5)** | *Untrusted · Linux MPU* | • Candidate action chunk: $\mathbf{a}_{t:t+H} \in \mathbb{R}^{H \times D}$<br>• Expiring Intent Lease: $\mathcal{L}_{\text{intent}} = \langle \mathbf{T}_{\text{target}}, \mathcal{B}, \dot{\mathbf{x}}_{\text{max}}, t_{\text{issue}}, \Delta t_{\text{TTL}} \rangle$<br>• Zero direct electrical authority over motor gate drivers |
| **MCU Permits (S1)** | *Trusted · Bare-Metal MCU* | • Validates physical invariants: joint limits, velocity bounds, stopping distance ($d_{\text{stop}} \le d_{\text{clear}}$)<br>• Projects proposals onto forward-invariant safe set: $h(\mathbf{x}) \ge 0$<br>• Sole privilege to write PWM registers and toggle gate enable (`EN_GATE`) |
...
| :--- | :--- | :--- | :--- |
| **Mental Anchor** | PyTorch, Linux, tensors, models, simulation | C/C++, FreeRTOS, memory-mapped I/O, timers, PWM | Dynamics, kinematics ($SE(3)$), PIDs, gearboxes |
| **Blindspot** | *Digital Sandbox:* Assuming simulation guarantees safety and exceptions are harmless. | *Static Automation:* Treating systems as rigid rules unable to parse open worlds. | *Closed-World:* Distrusting learned models; assuming the world is fully modeled in CAD. |
| **Breakthrough** | Process crash does **not** zero registers; timer PWM runs until MCU vetoes power. | Host large foundation models as untrusted MPU services without harming MCU timing. | Do not reject learned models; cage them in $1000\text{ Hz}$ Control Barrier Functions. |
...
Every architectural interface, firmware driver, and release qualification checkpoint established in this curriculum is anchored in four inescapable physical laws:

::: {.callout-law title="1 — Physical Causality and Irreversibility"}
Software state can be caught with a `try/catch` block, isolated in a container, or rolled back. Physical actions ($W_t \to W_{t+1}$) governed by mass, inertia, friction ($\mu$), and gravity ($g$) are permanent and irreversible. Every actuation permanently alters the physical environment, dissipates energy, and alters all future sensory observations endogenously ($A_t \to W_{t+1} \to O_{t+1}$).
:::

::: {.callout-law title="2 — Information Freshness (The Time Law)"}
The physical world does not pause while an AI model computes. Sensor observations age the microsecond photons strike the photodiode. Mean latency ($P_{50}$) is a dangerous illusion; tail latency ($P_{99}$), DRAM memory bus contention, and information age ($\Delta t$) dictate physical closed-loop stability and dynamic stopping distances ($d_{\text{stop}}$).
:::

::: {.callout-law title="3 — Proposal–Permission Privilege (The Architecture Law)"}
No stochastic, learned neural network (VLM, VLA, or Diffusion Policy) may hold direct motor authority. Learned models running on application processors (MPU) are *untrusted proposal services*. Real-time physical permission belongs exclusively to dedicated, zero-allocation microcontrollers (MCU) running deterministic mathematical safety envelopes and physical fallbacks.
:::

::: {.callout-law title="4 — Governed Change and Defensible Release (The Safety Law)"}
A physical agent shapes its own future data distribution. Naive training on raw operational logs without isolating safety interventions induces severe covariate shift and policy collapse. A cherry-picked video demo or simulation benchmark is not evidence of safety; defensible release requires cross-layer fault injection and a formal Claim-Argument-Evidence (CAE) safety case rendering an accountable **Deploy / Condition / Refuse** verdict.
:::
...
| Dimension | Archetype 1: Mobility | Archetype 2: Contact Manipulation | Archetype 3: Energy & Process |
| :--- | :--- | :--- | :--- |
| **Interaction** | Moving in Free Space | Touching & Shaping Matter | Regulating State Flows |
...
| **Silicon Tension**| Sensory DMA & $P_{99}$ latency tails | Multi-rate IPC & $\mathcal{C}^2$ jerk continuity | ADC sampling lag & thermal safe sets |
| **Real-World Anchors** | • Humanoids, Rovers<br>• Self-driving cars<br>• Drones | • Dexterous Manipulators<br>• Robotic Grippers<br>• Surgical Tools | • Battery Thermal Management<br>• Smart Grid Inverters<br>• Medical Infusion |
| **Curriculum Role** | Teaches Time, Latency & Stopping Distance | **Primary Teaching Spine & Lab Kit** (Arduino UNO Q) | Shows scope across the Analog–Digital Seam |
...
| **Active Medical Devices** | Neural glucose and drug rate predictor | Insulin volume, biochemical blood concentration | Hard-limited stepper clamp & dose watchdog |
| **Precision Manufacturing** | ViT toolpath & laser router | Spindle torque, laser beam thermal fluence | Axis limit enforcer & optical E-stop watchdog |
| **Autonomous Robotics** | VLA diffusion action chunk decoder | Joint torque, linkage inertia, contact force | 1 kHz bare-metal Control Barrier Function |
...
| **Joule Heating ($I^2 R$)** | Unlimited peak torque | Overheats at $T > 85^\circ\text{C}$ | Stall current damages stator coils |
| **Inductance ($\tau_e = L/R$)** | Torque responds in $0\text{ ms}$ | Current rises exponentially ($5\text{--}15\text{ ms}$) | Phase lag degrades stability |
| **Backlash ($\theta_{\text{bl}}$)** | Continuous kinematic chain | Deadband ($1\text{--}3\text{ arcmin}$) | Oscillation during reversals |
| **Jerk ($\dddot{\mathbf{q}}$)** | Step acceleration allowed | Jerk causes mechanical shock | Sheared pins and tooth fatigue |
...
| :--- | :--- | :--- |
| **Compute** | Multi-core ARM Cortex-A + NPU (Arduino UNO Q) | Single-core ARM Cortex-M / RISC-V (UNO Q MCU) |
| **OS** | Linux (Preempt-RT / Non-deterministic) | Bare-metal / FreeRTOS (Deterministic) |
| **Memory** | 2 GB LPDDR4 DRAM (Shared UMA Bus) | 256 KB Static SRAM / TCM (Zero Heap) |
...
| **Memory Policy** | Dynamic heap (`malloc`, PyTorch tensors) | Statically allocated SRAM buffers only |
| **Authority** | Untrusted (No direct actuator access) | **Trusted (Sole motor gate driver authority)** |
...
* **Chapter 3 (The Cognitive Dimensions):** Formalizes the five cognitive obligations, poses the twenty-five questions of the Co-Design Matrix, and establishes the multi-rate runtime. Freezes `FLOW-01`.
...
Every row represents one obligation of the embodied lifecycle, worked against the physical constraint columns that actually reach it:
...
::: {.callout-lab title="1: Closing the Causal Loop"}
