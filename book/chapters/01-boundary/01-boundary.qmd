...
[^pwm]: *Pulse-Width Modulation (PWM)* regulates effective electrical voltage to motors and heaters by pulsing full supply voltage on and off thousands of times per second, varying the fraction of time the signal is "high" (the duty cycle).
...
### The Landscape of Physical AI: Intelligent Systems in the Wild
...
Table @tbl-physical-landscape synthesizes this vast operational landscape across representative systems, showing how distinct physical media and actuation mechanisms share identical underlying computer systems challenges.
...
### The Three Canonical Archetypes: Distilling the Physical Landscape

How do we make scientific and engineering sense of this vast, dizzying diversity of machines—from a flying delivery drone to a five-hundred-kilowatt grid inverter? When we strip away application-specific mechanical jargon, every physical AI system maps directly to one of **Three Canonical Archetypes**:
...
In any physical feedback loop, the physical plant (a balancing robot, a flying drone, or a motor drive) has intrinsic natural dynamics characterized by an operating bandwidth or crossover frequency $\omega_c$[^crossover]. When software is inserted into this closed loop, every millisecond of end-to-end compute delay $\tau_{\text{delay}}$ (from photon arrival on the sensor to current ramp-up in the motor) injects an unavoidable phase lag into the feedback signal:

[^crossover]: The *crossover frequency* ($\omega_c$, measured in radians per second) characterizes the speed of the physical system's natural dynamics—how rapidly the physical plant responds to disturbances or control inputs.

$$\Delta \phi = -\omega_c \cdot \tau_{\text{delay}} \quad \text{[radians]}$$

Phase lag means the controller is always reacting to where the physical system *used to be* rather than where it *is now*. Think of trying to balance a broom upright on the palm of your hand: if your eyes take $200\text{ ms}$ to see the broom tilt and your hand takes another $200\text{ ms}$ to move, your hand moves to where the broom was in the past. Your corrective push arrives too late, pushing in the wrong direction and accelerating the fall.

In physical machines, as compute latency $\tau_{\text{delay}}$ grows, the system's phase margin[^phasemargin] (its safety buffer against oscillation) erodes. When phase margin drops to zero ($\text{PM} \le 0^\circ$), negative feedback flips into destructive positive feedback: minor sensor noise no longer dampens out—it amplifies exponentially into violent limit-cycle oscillations, motor saturation, and mechanical failure.

[^phasemargin]: *Phase Margin (PM)* is the safety cushion (in degrees or radians) between the system's current phase lag and the $180^\circ$ point where negative feedback flips into self-amplifying positive feedback.
...
When algorithmic predictions command physical actuators, this mental model collapses. The physical world possesses neither an exception handler nor an undo operation. The kinetic energy stored in a moving robotic link, the magnetic flux circulating through an electric motor, and the thermal energy accumulating in a power inverter do not pause when an application thread terminates. The following autopsy documents a representative, documented systems engineering failure mode where standard operating system process cleanup structurally failed to protect physical hardware.

::: {.callout-autopsy title="Latched Duty Cycle After Host Crash"}
* **System:** Six-degree-of-freedom articulated tabletop manipulator running an open-vocabulary Vision-Language-Action (VLA) policy on a Linux application processor (Nvidia Jetson / ARM Cortex-A) with direct memory-mapped I/O (MMIO) access to motor PWM timer registers.
* **Incident:** The manipulator struck an aluminum precision calibration jig at $1.2\text{ m/s}$, shattering the end-effector tooling and shearing the internal cycloidal drive pins in the wrist joint ($>\$4,500$ in hardware replacement costs).
* **Telemetry Snapshot:** At $T+12.410\text{ s}$, the high-level Python runtime issued a motor phase current target. At $T+12.412\text{ s}$, an unhandled specular reflection off an overhead fixture produced a zero-detection `NoneType` exception in the perception pipeline, triggering an unhandled fatal Python runtime crash (`SIGSEGV`). Hardware capture/compare registers on the peripheral timer bridge remained latched at $78\%$ duty cycle on an independent $100\text{ MHz}$ silicon clock tree.
* **Root Cause:** When the Linux kernel terminated the crashed process and reclaimed its virtual memory address space, physical MMIO peripheral timer registers were left untouched. Because timer counters reside on an independent silicon clock domain outside the kernel's virtual memory map, the hardware peripheral continued pulsing gate drivers at $20\text{ kHz}$, commutating full DC-bus current through the stator windings until physical impact arrested the arm.
* **Architectural Flaw:** Monolithic authority and unshielded hardware control—high-level learned software held unmediated electrical authority over motor timer peripherals with no independent, deterministic real-time permission supervisor (MCU watchdog / safe torque-off interlock) to de-energize the inverter upon host heartbeat expiration.
* **Provenance:** Grounded systems failure class. The mechanism and failure trajectory are drawn from documented physical robotic lab crashes and autonomous vehicle perception-actuation disconnect investigations (including NTSB Report NTSB/HAR-19/03 and industrial field service incident logs).
:::

In this incident, a six-degree-of-freedom (6-DoF)[^dof] manipulator running a vision-language-action (VLA) model[^vla] drove into a calibration fixture, fracturing its precision cycloidal speed-reducer gears[^cycloidal]. To understand why the manipulator in this incident continued driving into the fixture after the Python interpreter crashed, one must examine the physical silicon architecture of modern system-on-chip (SoC) hardware. In high-performance embedded processors, pulse-width modulation (PWM) peripherals and timer counters are implemented as discrete digital logic blocks connected to the system bus via peripheral bridges. These timer peripherals operate on dedicated hardware clock trees driven by independent phase-locked loops (PLLs) and bus prescalers. Once software initializes a timer counter and writes a duty cycle value into its capture and compare registers, the peripheral operates as an autonomous hardware state machine. It continuously counts clock cycles, toggles output pins, and generates gate-driver switching waveforms at frequencies of tens of kilohertz without requiring ongoing central processing unit (CPU) instruction execution.

[^dof]: *Degrees of Freedom (DoF)* refers to the number of independent joint axes the manipulator can move (typically 6 for full 3D position and orientation).
[^vla]: *Vision-Language-Action (VLA)* models are large multi-modal neural networks that ingest camera images and text instructions ("pick up the red mug") to predict robot joint motions.
[^cycloidal]: *Cycloidal drives* are high-precision, low-backlash speed-reduction gearboxes commonly used in robotic joints to step down high motor RPM into high joint torque.

When an unhandled exception or runtime panic terminates a user-space application process, the Linux kernel executes standard process teardown. The kernel invalidates the process's page directory, reclaims virtual memory allocations, closes open network sockets, and frees user-space file descriptors. However, physical memory-mapped I/O (MMIO)[^mmio] registers controlling external peripheral timers reside in hardware registers outside the operating system's virtual memory subsystem. The standard operating system kernel does not implicitly reconfigure, reset, or de-energize external peripheral timer registers upon user-space process termination. Because the timer peripheral resides on an independent silicon clock domain, its internal digital counters continue cycling, holding the gate-driver control lines at the last latched duty cycle. The motor inverter's solid-state H-bridge[^hbridge] continues commutating electrical power, phase currents continue flowing through the stator windings, and electromagnetic torque accelerates the mechanical linkage until physical impact arrests the motion.
...
[^hbridge]: An *H-bridge* is an electronic circuit of four solid-state transistor switches that allows voltage to be applied across a motor winding in either direction, controlling rotation direction.
...
An *expiring intent lease*[^intentlease] acts as a time-bounded spatial and velocity contract issued by the cognitive brain (for example, "Authorized to reach for target object within workspace envelope $\mathcal{B}$ at speed $\le 0.5\text{ m/s}$ for the next $500\text{ ms}$"). If the cognitive processor freezes or misses a deadline, the lease expires passively, and the permission authority safely halts the robot.
...
| :--- | :--- | :--- |
| **MPU Proposes** | Untrusted · Linux MPU / NPU | Emits candidate action chunks $\mathbf{a}_{t:t+H}$ within an expiring intent lease; emits heartbeats at $\le 20\text{ ms}$ periods; possesses zero direct electrical authority over motor gate drivers. |
| **MCU Permits** | Trusted · Bare-Metal MCU / RTOS | Validates joint limits, velocity bounds, and dynamic stopping clearances; projects or rejects proposals under barrier invariants; holds sole privilege to write PWM registers and toggle gate enable lines. |
...
* *Execution $\times$ Stopping (Chapter 8):* Dedicated real-time microcontrollers must evaluate dynamic stopping inequalities ($d_{\text{stop}} \le d_{\text{clearance}}$) and solve quadratic-program Control Barrier Functions (CBFs—mathematical certificates that guarantee the state never exits a safe envelope) in under one millisecond to project untrusted candidate actions onto forward-invariant safe sets before gate drivers are energized.
...
::: {.callout-fallacy title="The Digital Sandbox"}
* **The Fallacy:** Traditional software error-handling abstractions, such as `try/catch` exception blocks or operating system process isolation, protect physical hardware during runtime crashes.
* **The Physical Reality:** In modern System-on-Chip architectures, peripheral timer registers operate on autonomous silicon clock trees. When a user-space application thread crashes, the operating system kernel reclaims virtual memory but leaves memory-mapped I/O timer registers latched at their last written duty cycle. The motor inverter continues commutating electrical power, driving high stall currents through stator windings until mechanical linkages strike physical obstacles or thermal runaway destroys the actuator.
:::

::: {.callout-fallacy title="The Monolithic End-to-End Architecture"}
* **The Fallacy:** Scaling a single deep neural network from raw sensor pixels directly to motor pulse-width modulation (PWM) duty cycles eliminates the need for classical control theory, real-time operating systems, and hardware safety supervisors.
* **The Physical Reality:** Because deep neural networks are statistical function approximators susceptible to out-of-distribution visual artifacts, dynamic occlusions, and specular reflections, and general-purpose operating systems exhibit multi-millisecond tail latency spikes ($P_{99} > 100\text{ ms}$), granting an end-to-end model unmediated electrical authority over motor inverters guarantees eventual physical destruction.
:::

::: {.callout-fallacy title="The Average Throughput Metric"}
* **The Fallacy:** Achieving a high median frame rate ($P_{50}$) or average inference throughput guarantees closed-loop physical safety.
* **The Physical Reality:** In continuous dynamical systems, closed-loop stability and dynamic stopping distances are governed strictly by worst-case latency tails ($P_{99}$ and $P_{99.9}$). A single $250\text{ ms}$ scheduling blackout caused by memory bus contention or garbage collection while a vehicle travels at $2.0\text{ m/s}$ translates into half a meter of blind, unguided motion, transforming an otherwise safe trajectory into an unavoidable collision.
:::

::: {.callout-fallacy title="Simulation-to-Real Equivalence"}
* **The Fallacy:** A learned policy achieving high task success in rigid-body physics simulation will transfer directly to benchtop hardware without architectural modification.
* **The Physical Reality:** Physics simulation engines routinely abstract away electromagnetic inductance, mechanical transmission backlash, structural compliance, memory bus contention, and irreversible Joule heating. On physical hardware, unmodeled phase lags and discontinuous torque commands generate severe mechanical jerk, excite chassis resonances, chip gearbox teeth, and induce thermal faults.
:::
