...
\begin{tikzpicture}[
  ...
  \node[card] (p1) {
    ...
    {\normalsize\bfseries\color{ethdarkblue}Disembodied ML}\\[2pt]
    {\scriptsize\bfseries\color{ethslate}The Cloud Era}\\[6pt]
    ...
    $\bullet$ \textbf{Physical Action:} None (Bits on screen)
    }
  };

  % --- Phase 2: Edge Perception ---
  \node[card, right=0.60in of p1] (p2) {
    {\scriptsize\bfseries\color{ethpetrol}\colorbox{ethpetrol!12}{\,PHASE 2 $\cdot$ 2018--2023\,}}\\[5pt]
    {\normalsize\bfseries\color{ethdarkblue}Edge Perception}\\[2pt]
    {\scriptsize\bfseries\color{ethpetrol}The TinyML Era}\\[6pt]
    {\scriptsize\color{ethslate}
    \textbf{Systems Focus:}\\
    Compressing neural models onto microcontrollers\\[5pt]
  ...
  \node[card, right=0.60in of p2] (p3) {
    {\scriptsize\bfseries\color{ethblue}\colorbox{ethblue!12}{\,PHASE 3 $\cdot$ 2023--2026\,}}\\[5pt]
    {\normalsize\bfseries\color{ethdarkblue}Deliberation}\\[2pt]
    {\scriptsize\bfseries\color{ethblue}The Foundation Era}\\[6pt]
    {\scriptsize\color{ethslate}
    \textbf{Systems Focus:}\\
    Open-vocabulary spatial reasoning via multimodal models\\[5pt]
    ...
    $\bullet$ \textbf{Failure Mode:} Hallucination, $P_{99}$ spikes\\
    $\bullet$ \textbf{Physical Action:} Unverified proposals
    }
  };

  % --- Phase 4: Physical AI Systems ---
  \node[frontiercard, right=0.60in of p3] (p4) {
    {\scriptsize\bfseries\color{harvardcrimson}\colorbox{harvardcrimson!15}{\,PHASE 4 $\cdot$ NOW (THE FRONTIER)\,}}\\[5pt]
    {\normalsize\bfseries\color{harvardcrimson}Physical AI Systems}\\[2pt]
    {\scriptsize\bfseries\color{harvardcrimson}Closed-Loop Actuation}\\[6pt]
    {\scriptsize\color{ethslate}
    \textbf{Systems Focus:}\\
    ...
    $\bullet$ \textbf{Boundary:} Delegated physical control\\
  ...
  \draw[->, line width=1.5pt, ethslate!60] (p1.east) -- node[above=4pt, font=\sffamily\bfseries\scriptsize\color{ethslate}, midway]{Compress} (p2.west);
  \draw[->, line width=1.5pt, ethpetrol!70] (p2.east) -- node[above=4pt, font=\sffamily\bfseries\scriptsize\color{ethpetrol}, midway]{Reason} (p3.west);
  \draw[->, line width=2pt, harvardcrimson!90] (p3.east) -- node[above=4pt, font=\sffamily\bfseries\scriptsize\color{harvardcrimson}, midway]{Close Loop} (p4.west);
