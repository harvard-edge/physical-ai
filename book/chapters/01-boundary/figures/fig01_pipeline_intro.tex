...
\begin{tikzpicture}[
  ...
  \node[neutralorgan, text width=7.4in, fill=subtlebg, draw=bordergray] (s7) {
    \textbf{\color{darkslate}\faIcon{balance-scale}\; 7. GOVERNANCE \& RELEASE GATE} $\cdot$ STPA Hazard Analysis $\cdot$ Bumpless Override $\cdot$ Defensible Release
  };

  % --- TOP ROW: Organs 1, 2, 3 ---
  \node[neutralorgan, text width=2.22in, below=0.22in of s7.south west, anchor=north west] (s1) {
    \textbf{\color{darkslate}\faIcon{satellite-dish}\; 1. SENSING}\\
  ...
  \node[neutralorgan, text width=2.22in, right=0.37in of s1] (s2) {
    \textbf{\color{darkslate}\faIcon{eye}\; 2. PERCEPTION}\\
  ...
  \node[neutralorgan, text width=2.22in, right=0.37in of s2] (s3) {
    \textbf{\color{darkslate}\faIcon{database}\; 3. MEMORY}\\
  ...
  \node[neutralorgan, text width=3.51in, below=0.25in of s3.south east, anchor=north east] (s4) {
    \textbf{\color{darkslate}\faIcon{brain}\; 4. REASONING (System 2 $\cdot$ MPU)}\\
  ...
  \node[neutralorgan, text width=3.51in, left=0.38in of s4] (s5) {
    \textbf{\color{darkslate}\faIcon{network-wired}\; 5. PLANNING (System 1.5 $\cdot$ MPU/NPU)}\\
  ...
  \node[neutralorgan, text width=7.4in, below=0.25in of s5.south west, anchor=north west] (s6) {
    \textbf{\color{darkslate}\faIcon{shield-alt}\; 6. REFLEX (System 1 $\cdot$ MCU)}\\
  ...
  \node[neutralorgan, draw=bordergray, fill=subtlebg, text width=7.4in, below=0.22in of s6.south west, anchor=north west] (world) {
    \textbf{\color{darkslate}\faIcon{cogs}\; THE PHYSICAL WORLD ($W_t \to W_{t+1}$)} $\cdot$ Kinetic Momentum $\cdot$ Joule Heat $\cdot$ Friction $\cdot$ Collision Dynamics
  };
  ...
  \draw[neutralarrow] (world.west) -- ++(-0.35in, 0) |- (s1.west) 
    node[pos=0.25, above, rotate=90, font=\sffamily\scriptsize\bfseries, text=bodytext] {\faIcon{sync-alt}\; Closed Sensory-Motor Feedback ($O_{t+1}$)};
