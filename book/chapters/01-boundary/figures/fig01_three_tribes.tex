...
\begin{tikzpicture}[
  ...
  \node[tribecard, draw=ethblue!70] (brain) {
    ...
    {\scriptsize\bfseries\color{ethslate}Computer Science / AI Foundation}\\[5pt]
    ...
    \textit{The Digital Sandbox Illusion}---assuming simulation proof guarantees safety and software exceptions are harmless.}
  };

  % --- TRIBE 2: The Nervous System (Embedded / ECE) ---
  \node[tribecard, draw=ethpetrol!80, right=0.3in of brain] (nervous) {
    ...
    \textbf{\color{harvardcrimson}The Critical Blindspot:}\\
    \textit{The Static Automation Illusion}---treating systems as rigid, static loops unable to parse unstructured open worlds.}
  };

  % --- TRIBE 3: The Body & Control (Robotics / Mechanical) ---
  \node[tribecard, draw=ethbronze!80, right=0.3in of nervous] (body) {
    ...
    \textbf{\color{harvardcrimson}The Critical Blindspot:}\\
    \textit{The Closed-World Illusion}---distrusting learned models as black boxes; failing when lighting or physical objects deviate.}
  };

  % --- SYNTHESIS BANNER: Physical AI Systems ---
  \node[synthesiscard, below=0.35in of nervous.south] (synthesis) {
    \begin{minipage}{0.99\linewidth}
      ...
      \begin{tabular}{p{2.3in} p{2.3in} p{2.3in}}
        ...
        \scriptsize High-capacity VLMs \& Diffusion ACT emit candidate action chunks ($p_t$) on Linux MPU. &
        \scriptsize Lock-free SRAM ring buffers and hardware watchdogs bound tail latency ($P_{99}$). &
        \scriptsize 1 kHz MCU projects proposals onto safe sets $\Pi_{\mathcal{U}_{\text{safe}}}(p_t)$ via CBFs before gate drive.
      \end{tabular}\\[7pt]
  ...
  \draw[->, line width=1.1pt, draw=ethblue] (brain.south) -- ++(0,-0.15in) -| ($(synthesis.north west) + (1.2in, 0)$)
    node[pos=0.25, arrowlabel, text=ethblue] {Semantic Proposals ($p_t$)};

  \draw[->, line width=1.1pt, draw=ethpetrol] (nervous.south) -- (synthesis.north)
    node[pos=0.45, arrowlabel, text=ethpetrol] {Multi-Rate IPC \& Watchdogs};

  \draw[->, line width=1.1pt, draw=ethbronze] (body.south) -- ++(0,-0.15in) -| ($(synthesis.north east) + (-1.2in, 0)$)
