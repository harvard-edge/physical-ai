...
\begin{tikzpicture}[
  ...
  \node[allactiveorgan, text width=7.4in] (s7) {
    \textbf{\faIcon{balance-scale}\; 7. GOVERNANCE \& RELEASE GATE} $\cdot$ STPA Hazard Mitigation $\cdot$ Bumpless Override $\cdot$ Defensible Release Case
  };
  ...
  % --- LOWER ROW: Organ 6 ---
  \node[allactiveorgan, text width=7.4in, below=0.42in of s5.south west, anchor=north west] (s6) {
  ...
  % --- PHYSICAL WORLD ROW ---
  \node[activeorgan, draw=harvardcrimson!60, fill=white, text width=7.4in, below=0.22in of s6.south west, anchor=north west] (world) {
