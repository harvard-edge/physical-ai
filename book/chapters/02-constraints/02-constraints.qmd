...
When software leaves the cloud and takes physical form, the relationship between computation and time undergoes a fundamental inversion. In the cloud, compute operates within an elastic, turn-based reality where a delayed inference merely prolongs a user's wait behind glass. In the physical realm, however, time is an unyielding, continuous dynamical state variable. The physical world sets the clock, and it waits for no processor. When an embodied agent is late, the physical universe continues to evolve, rendering computed action commands dangerously stale before they ever manifest as physical torque.
...
Together, these five columns form the immutable bedrock upon which all embodied intelligence must be engineered. They are not disconnected edge cases; they are the central architectural drivers of Physical AI. This chapter systematically instruments and quantifies these five constraints:

1. **Section 2.2 (Time & Information Freshness):** We deconstruct the 7-stage sense-to-actuation latency waterfall and calculate the geometric Freshness Wall ($\Delta t_{\text{wall}}$) where internal model belief diverges from physical reality.
2. **Section 2.3 (Inertia & Dynamic Stopping Envelopes):** We derive two-phase dynamic stopping kinematics ($d_{\text{stop}} = d_{\text{react}} + d_{\text{brake}}$) under worst-case tail latency spikes ($P_{99}$).
3. **Section 2.4 (Actuation Dynamics, Inductance & Jerk):** We analyze motor electrical time constants ($\tau_e = L/R$) and enforce $\mathcal{C}^2$ quintic jerk bounding ($\|\dddot{\mathbf{q}}\| \le \dddot{q}_{\text{max}}$) to protect precision gearboxes from stress fracture.
4. **Section 2.5 (Energy & Thermals):** We model stator winding Joule heating ($I^2Rt$) and silicon Dynamic Voltage and Frequency Scaling (DVFS) thermal cliffs in sealed robotic enclosures.
5. **Section 2.6 (Silicon Determinism & The Iron Law):** We resolve Unified Memory Architecture (UMA) crossbar contention via hardware AXI QoS registers and enforce compile-time static SRAM execution (`malloc = 0`).
6. **Section 2.7 & 2.8 (The Requirements Ledger & Handoff):** We compile the empirical requirements ledger to freeze physical invariants, establishing the quantitative baseline for Part I before introducing cognitive agency in Chapter 3.
...
![**Hardware-Triggered Metrology Topology.** Escaping software timestamp delusions via logic analyzer GPIO toggles and shunt current probes. Physical ground-truth instrumentation bypasses operating system abstractions to measure true end-to-end latency directly on electrical traces.](figures/fig02_metrology_setup.svg){#fig-metrology-setup width=100%}

To measure true physical latency, engineers must measure physical electrical transitions directly (@fig-metrology-setup): from the hardware strobe line when light strikes the camera sensor, across memory buses and processor mailboxes, all the way to the analog current probe measuring stator phase current rise on the motor inverter.
...
In frequency-domain control theory, pure transport delay $t_{\text{delay}}$ rotates the open-loop transfer function phase angle by:

$$\Delta \phi = -\omega_c t_{\text{delay}} \cdot \frac{180^\circ}{\pi}$$

At a typical closed-loop system bandwidth (control crossover frequency) of $\omega_c = 20\text{ rad/s}$ ($\approx 3.2\text{ Hz}$), a $126\text{ ms}$ delay injects a massive $144.4^\circ$ phase lag. In physical terms, phase lag means the actuator applies corrective torque based on where the system was in the past rather than where it is now; when phase lag approaches $180^\circ$, negative feedback turns into positive feedback—the controller pushes in the direction of disturbance rather than opposing it, triggering violent, self-reinforcing oscillations that destroy stability [@astrom2021feedback].
...
Even if an aging sensory observation successfully detects an obstacle before the Freshness Wall collapses, the system encounters an equally unyielding physical barrier: moving bodies possess mass and kinetic momentum. In digital software, resetting a velocity variable to zero is an instantaneous, single-cycle memory write. In physical mechanics, moving mass ($p = mv$, $E_k = \frac{1}{2}mv^2$) cannot halt on a dime; bringing mass to rest requires applying opposing force over finite time and distance, dissipating kinetic energy into the environment.
...
Enforcing dynamic stopping distances or tracking high-speed trajectory splines implicitly assumes that motor drives can instantly materialize whatever forces the controller requests. In physical reality, actuators are not idealized mathematical torque sources. They must build electromagnetic flux against stator winding impedance (electrical resistance $R$ and inductance $L$), transmit torque through elastic gear teeth, and continuously respect mechanical structural limits.
...
When applying a step voltage (assuming rotor speed $\omega$ remains roughly constant over this rapid electrical transient), phase current rises exponentially: 

$$I(t) = \frac{V_{\text{bus}} - K_e\omega}{R}\left(1 - e^{-t/\tau_e}\right)$$

Achieving 95% commanded torque consumes approximately three electrical time constants ($3\tau_e \approx 6\text{ to } 15\text{ ms}$). Furthermore, as rotor speed $\omega$ increases, the motor acts as an electrical generator; this opposing *back-EMF* ($K_e\omega$) fights the incoming bus voltage, reducing available torque responsiveness at high operating velocities.

### Transmissions and Jerk Continuity

Transmissions (such as harmonic drives, planetary reducers, and cycloidal drives) transfer torque through gear teeth subject to shear stress limits ($\sigma_{\text{shear}} \le \sigma_{\text{yield}}$). If a discrete neural policy emits step changes in trajectory waypoints (joint position targets $\mathbf{q}$), commanded acceleration jumps discontinuously ($\Delta \mathbf{a} \ne 0$), implying unbounded mechanical jerk:

$$\mathbf{j}(t) = \dddot{\mathbf{q}}(t) = \frac{d^3\mathbf{q}}{dt^3} = \frac{d\mathbf{a}}{dt} \longrightarrow \infty$$

Mathematically, a step change in acceleration creates an impulsive delta-function jerk spike ($j(t) = \Delta a \cdot \delta(t)$). Unbounded jerk triggers three severe physical failure modes:
...
Even when trajectory profiles maintain pristine $\mathcal{C}^2$ continuity to protect gearboxes from mechanical shock, sustained actuation creates an invisible, compounding thermodynamic consequence. Every actuation command requests physical work, and no energy conversion is perfectly conservative. Resistive losses in motor copper coils and power semiconductor switches generate irreversible Joule heat ($P_{\text{loss}} = I^2 R$).
...
Because physical thermistors mounted on motor casings exhibit a $5\text{--}30\text{ s}$ spatial conduction lag (the time required for heat to diffuse from inner copper windings through iron laminations to the exterior chassis), the real-time microcontroller cannot wait for thermistor readings during sudden stall events. Instead, the firmware maintains an $I^2 t$ thermal energy accumulator in static SRAM on every 1 kHz tick—operating as a digital leaky integrator with a zero-bound ambient baseline:
...
The five physical columns—time freshness, inertial momentum, actuator dynamics, thermal dissipation, and silicon determinism—are not disconnected hardware quirks; together, they define the exact physical boundary within which learned intelligence is permitted to act. In classical engineering, every safety-critical machine operates under a certified specification sheet. In Physical AI, deploying neural policies without an empirical, hardware-verified operating ledger invites physical destruction. Before any autonomous agent is permitted to energize motor drivers, the engineering team must measure, calculate, and freeze the system's physical operating invariants in a formal *Requirements Ledger*.
