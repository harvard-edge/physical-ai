...
Together, these five columns form the immutable bedrock upon which all embodied intelligence must be engineered. They are not disconnected edge cases; they are the central architectural drivers of Physical AI. 

To understand how hardware bounds intelligent action, this chapter systematically instruments each constraint from first principles. We begin by deconstructing the seven-stage sense-to-actuation latency waterfall (Section 2.2), calculating the geometric Freshness Wall where internal model belief diverges from physical reality. We then derive two-phase dynamic stopping kinematics (Section 2.3), demonstrating why worst-case latency spikes expand stopping distance quadratically. Next, we analyze motor electrical time constants and enforce jerk bounding to protect precision gearboxes from stress fracture (Section 2.4), before modeling stator winding Joule heating and dynamic thermal derating in sealed enclosures (Section 2.5). Finally, we resolve memory bus contention through hardware quality-of-service arbitration while enforcing compile-time static memory execution (Section 2.6), compiling the empirical requirements ledger (Section 2.7) to freeze physical invariants before introducing cognitive agency in the next chapter (Section 2.8).
...
During Phase 1 (the linear reaction phase, $v_0 \cdot \Delta t_{\text{delay}}$), the robot is mechanically blind to the obstacle while computation transpires. It coasts forward at velocity $v_0$ with zero deceleration, exposing the fundamental bridge between computer systems and physics: computational delay is linear unguided travel through space. Once braking begins in Phase 2 (the quadratic braking phase, $\frac{v_0^2}{2 a_{\text{max}}}$), the machine decelerates at its maximum mechanical limit (deceleration magnitude $a_{\text{max}} > 0$), dissipating kinetic energy over a distance that scales quadratically with initial velocity. In mobile locomotion, maximum deceleration is governed by Coulomb surface friction ($a_{\text{max}} \le \mu g$). Under combined braking and steering maneuvers, total acceleration is bounded by Kamm's friction circle ($\sqrt{a_{\text{long}}^2 + a_{\text{lat}}^2} \le \mu g$), where lateral cornering forces directly subtract from available longitudinal stopping authority.
