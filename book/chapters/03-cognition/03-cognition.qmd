...
::: {.callout-autopsy title="The Cadence Desynchronization and Latency Overrun"}
* **System:** Level 4 Automated Driving System operating on public roadways, equipped with roof-mounted 64-beam LiDAR, 360° radar array, and forward optical cameras [@ntsb2019uber].
* **Incident:** While traveling at $43.5\text{ mph}$ ($19.4\text{ m/s}$) on an unlit roadway, the vehicle struck a pedestrian crossing the road with zero automated braking initiated prior to impact.
* **Telemetry Snapshot:** Over the $5.6\text{ seconds}$ preceding collision, the perception system repeatedly toggled classification between unknown object, vehicle, and bicycle across consecutive $10\text{ Hz}$ cycles. Each re-classification reset the tracking history and recomputed expected trajectory vectors. At $T - 1.2\text{ s}$ before impact ($d_{\text{clearance}} = 23.3\text{ m}$), when the system finally projected an unavoidable collision, an unmitigated software suppression policy enforced a $1000\text{ ms}$ delay before dispatching braking commands to prevent "erratic braking" false alarms.
* **Root Cause:** Asynchronous cadence mismatch and unmitigated software suppression delay—the high-level perception model's classification thrashing induced tracking jitter, and the motion planning layer suppressed emergency braking during the critical stopping distance window without an independent real-time safety envelope.
* **Architectural Flaw:** Monolithic authority and software suppression policy with zero independent microcontroller-level dynamic stopping supervisor ($d_{\text{clearance}} \ge d_{\text{stop}}$) and lack of expiring intent leases.
