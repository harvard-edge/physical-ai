...
Decoupling cognition across three distinct cadences resolves the compute-versus-physics conflict, but introduces a new systems challenge: how do asynchronous runtimes operating at vastly different frequencies exchange data safely without race conditions, stale buffers, or missed deadlines [@saltzer1984end]?
...
where $h(\mathbf{x}) \ge 0$ represents Control Barrier Function safety constraints (such as maintaining dynamic stopping distance $d_{\text{clearance}} \ge d_{\text{stop}}$) [@ames2019control]. The permission authority alone holds the electrical privilege to configure PWM timer registers and toggle gate drivers.
...
Action chunking amortizes computational delay by predicting multi-step trajectories $\mathbf{A}_{t:t+H}$ across a rolling horizon $H = 16\text{--}64$ steps ($300\text{--}1000\text{ ms}$) [@zhao2023learning]. The high-rate microcontroller consumes waypoints from the active chunk at $1\text{ kHz}$, fitting $\mathcal{C}^2$-continuous splines and applying temporal ensembling to blend overlapping predictions seamlessly.
