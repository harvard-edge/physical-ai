...
\begin{tikzpicture}[
  ...
  \node[flowbox, draw=ethblue] (b1) at (0.40in, 0in) {
    ...
    {\tiny\textbf{\color{ethblue}Ch 5}}
  };

  \node[flowbox, draw=ethblue!80] (b2) at (1.60in, 0in) {
  ...
  \node[flowbox, draw=ethpurple] (b3) at (2.80in, 0in) {
    ...
    {\tiny\textbf{\color{ethpurple}Ch 6}}
  };

  \node[flowbox, draw=ethbronze] (b4) at (4.00in, 0in) {
  ...
  \node[flowbox, draw=ethbronze!90] (b5) at (5.20in, 0in) {
    ...
    {\tiny\textbf{\color{ethbronze}Ch 8}}
  };



  % System 1 Safety & Plant
  \node[flowbox, draw=safeTeal] (b6) at (6.70in, 0in) {
    {\tiny\bfseries\color{safeTeal} STAGE 6}\\[2pt]
    {\scriptsize\bfseries Safety Reflex}\\[3pt]
    {\tiny\color{ethslate}$1\text{ kHz}$ CBF Filter\\[1pt]\& Stopping Veto}\\[5pt]
    {\tiny\textbf{\color{safeTeal}Ch 9}}
  };

  \node[flowbox, draw=harvardcrimson] (b7) at (7.90in, 0in) {
    ...
    {\tiny\textbf{\color{harvardcrimson}Ch 10}}
  };
  ...
  \draw[dashed, line width=1.2pt, draw=ethbronze] (5.95in, 0.65in) -- (5.95in, -0.65in);
  \node[font=\sffamily\bfseries\tiny, fill=white, draw=ethbronze, rounded corners=2pt, inner sep=2pt, text=ethbronze, align=center] at (5.95in, 0.85in) {PROPOSAL--PERMISSION\\BOUNDARY};
  ...
  \node[draw=harvardcrimson, fill=harvardcrimson!5, rounded corners=5pt, line width=1pt, text width=8.90in, inner sep=6pt, align=center] (world) at (4.45in, -1.85in) {
    ...
    {\tiny\color{ethslate}Mechanical Mass Displaced $\quad\bullet\quad$ Kinetic Momentum ($\mathbf{p} = m\mathbf{v}$) Mutated $\quad\bullet\quad$ Joule Heat ($I^2R$) Dissipated $\quad\bullet\quad$ Mechanical Stresses Applied}
  };
