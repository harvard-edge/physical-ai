...
\begin{tikzpicture}[
  ...
  \node[allactiveorgan, text width=6.8in] (s7) {
  ...
  \node[allactiveorgan, text width=2.0in, below=0.22in of s7.south west, anchor=north west] (s1) {
    \textbf{1. SENSING}\\
    {Photons / Voltages to DMA $\cdot$ MIPI/SPI}
  };

  \node[allactiveorgan, text width=2.0in, right=0.3in of s1] (s2) {
    \textbf{2. PERCEPTION}\\
    {ViT Encoders $\cdot$ DINOv2 $\cdot$ 3D Tokens}
  };

  \node[allactiveorgan, text width=2.0in, right=0.3in of s2] (s3) {
    \textbf{3. MEMORY}\\
    {Temporal Belief $\cdot$ $SE(3)$ Trees $\cdot$ JEPAs}
  };

  % --- MIDDLE ROW: Organs 5, 4 ---
  \node[allactiveorgan, text width=3.15in, below=0.25in of s3.south east, anchor=north east] (s4) {
    \textbf{4. REASONING (System 2 $\cdot$ MPU)}\\
    {Vision-Language Models $\cdot$ 3D Goals $\cdot$ Leases ($t_{\text{expire}}$)}
  };

  \node[allactiveorgan, text width=3.15in, left=0.4in of s4] (s5) {
    \textbf{5. PLANNING (System 1.5 $\cdot$ MPU/NPU)}\\
    {Diffusion Policies $\cdot$ ACT Chunking ($H$-steps) $\cdot$ Jerk Continuity}
  };

  % --- LOWER ROW: Organ 6 ---
  \node[allactiveorgan, text width=6.8in, below=0.38in of s5.south west, anchor=north west] (s6) {
  ...
  % --- PHYSICAL WORLD ROW ---
  \node[activeorgan, draw=harvardcrimson!60, fill=white, text width=6.8in, below=0.22in of s6.south west, anchor=north west] (world) {
