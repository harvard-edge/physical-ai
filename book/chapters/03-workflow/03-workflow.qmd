...
1. **Trace the End-to-End Agent Thinking Workflow:** Map an embodied task across its nine lifecycle stations—from open-vocabulary prompt ingestion down through 3D spatial tokenization, latent world modeling, action chunk decoding, real-time safety projection, motor PWM switching, and endogenous feedback.
2. **Deconstruct the Three Cadences of Intelligence:** Architect the multi-rate hierarchy decoupling slow semantic deliberation (System 2, $0.5\text{--}2\text{ Hz}$) from intermediate trajectory decoding (System 1.5, $20\text{--}50\text{ Hz}$) and bare-metal safety reflexes (System 1, $1000\text{ Hz}$).
...
5. **Navigate the Modular Blueprint for Parts 2 and 3:** Connect each station in the overarching agent workflow to its dedicated pipeline organ chapter (Chapters 4–8) and silicon placement chapter (Chapters 9–11).
...
Before an engineering team can build individual sensory-motor modules, they must understand how information flows through the entire agent from end to end. An embodied physical AI system is an integrated cognitive and physical pipeline comprising nine distinct lifecycle stations:
...
## The Three Cadences of Physical Intelligence

To execute these nine stations efficiently, modern physical agents decouple execution into **Three Distinct Temporal Cadences**:
...
Now we bring the two halves of our foundation together: the hard physical and temporal laws established in **Chapter 2** and the cognitive agent workflow established in **Chapter 3**.

Physical AI is the engineering discipline born from the violent collision of these two worlds pulling with maximum force in **diametrically opposite directions**:
...
How do these two opposing worlds operate on the same machine without tearing the physical system apart? Physical AI systems rely on **Three Formal Architectural Contracts**:
...
With the foundational conflict crystallized and the peace treaties established, the entire remainder of the book becomes a modular engineering journey to build, isolate, and qualify each organ of the physical agent:
...
::: {.callout-decision}
### Dossier Delta: Multi-Rate Workflow Charter (`FLOW-01`)
...
Part I has established the physical foundation of embodied intelligence:
1. **Chapter 1 (Physical Causality):** Software moves matter; physical energy cannot be undone; untrusted models must be separated from motor drivers.
2. **Chapter 2 (Time, Freshness & Latency):** The world sets the clock; information freshness decays; tail latency ($P_{99}$) governs stopping distance.
3. **Chapter 3 (The Agent Workflow):** The 9-station dataflow and 3-cadence hierarchy reveal the fundamental conflict between deliberation and physical dynamics.

In **Part II (Chapters 4–8)**, we take this conflict head-on. We systematically construct, isolate, and bind together each of the **Five Canonical Sensory-Motor Organs** across the Proposal–Permission boundary: starting with **Perception and Ingestion Taxes** in Chapter 4.
