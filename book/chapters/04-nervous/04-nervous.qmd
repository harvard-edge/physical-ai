...
[^fn-math-zoh-phaselag]: The zero-order hold (ZOH) transfer function $G_{\text{zoh}}(j\omega) = e^{-j \omega T_s / 2} \cdot T_s \operatorname{sinc}(\frac{\omega T_s}{2\pi})$ introduces an unavoidable half-sample phase lag $\Delta \phi = -\frac{\omega T_s}{2}$ radians. Discrete sampling directly degrades feedback phase margin independently of computational delay $T_d$.

[^fn-hw-dma-coherence]: On heterogeneous SoCs without hardware cache coherency, software must execute cache clean and invalidate barriers (`SCB_CleanInvalidateDCache`) around peripheral DMA transfers. An omitted flush leaves stale cache lines, silently corrupting motor buffers without raising CPU fault traps.
