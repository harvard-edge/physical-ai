...
\begin{tikzpicture}[
  ...
  \node[draw=ethdarkblue, fill=ethdarkblue!5, rounded corners=5pt, line width=1pt, text width=8.80in, inner sep=7pt, align=center] (title) at (4.40in, 0) {
    {\normalsize\bfseries\color{ethdarkblue}\faIcon{microchip}\;\; UMA MEMORY BUS CONTENTION \& THE PRE-INFERENCE INGESTION TAX}\\[2pt]
    {\scriptsize\color{ethslate}How Unthrottled Sensor DMA Writes Starve Neural Weight Streaming on Shared LPDDR5 Memory Architectures}
  };

  % Three Subsystems on the SoC
  \node[enginebox, draw=harvardcrimson!80] (cam) at (0.95in, -0.65in) {
    {\scriptsize\bfseries\color{harvardcrimson}\faIcon{video}\; SENSOR INGESTION}\\[3pt]
  ...
  \node[enginebox, draw=ethbronze!90] (npu) at (4.40in, -0.65in) {
    {\scriptsize\bfseries\color{ethbronze}\faIcon{brain}\; NEURAL ACCELERATOR}\\[3pt]
  ...
  \node[enginebox, draw=ethpetrol!90] (cpu) at (7.85in, -0.65in) {
    {\scriptsize\bfseries\color{ethpetrol}\faIcon{tachometer-alt}\; REAL-TIME CONTROL}\\[3pt]
  ...
  \node[draw=ethdarkblue, fill=ethdarkblue!10, rounded corners=4pt, line width=1.1pt, text width=8.80in, minimum height=0.55in, inner sep=6pt, align=center] (bus) at (4.40in, -2.45in) {
    {\small\bfseries\color{ethdarkblue}\faIcon{network-wired}\; SHARED LPDDR5 SYSTEM BUS \& CROSSBAR INTERCONNECT}\\[2pt]
  ...
  \node[draw=alertRed!70, fill=alertRed!5, rounded corners=4pt, line width=0.8pt, text width=4.20in, minimum height=1.35in, inner sep=6pt, align=left] (naive) at (2.20in, -3.85in) {
    {\scriptsize\bfseries\color{alertRed}\faIcon{times-circle}\; NAIVE BUFFERING (High Tax \& Latency Spikes)}\\[3pt]
    {\tiny\color{ethslate}
  ...
  \node[draw=safeTeal!80, fill=safeTeal!5, rounded corners=4pt, line width=0.8pt, text width=4.20in, minimum height=1.35in, inner sep=6pt, align=left] (zerocopy) at (6.60in, -3.85in) {
    {\scriptsize\bfseries\color{safeTeal}\faIcon{check-circle}\; ZERO-COPY DMA RING BUFFER (OBS-01 Architecture)}\\[3pt]
    {\tiny\color{ethslate}
