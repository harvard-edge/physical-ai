...
\begin{tikzpicture}[
  ...
  \node[activeorgan, text width=2.0in, below=0.22in of s7.south west, anchor=north west] (s1) {
    \textbf{1. SENSING}\\
    {Photons / Voltages to DMA $\cdot$ MIPI/SPI}
  };

  \node[activeorgan, text width=2.0in, right=0.3in of s1] (s2) {
    \textbf{2. PERCEPTION}\\
  ...
  % --- LOWER ROW: Organ 6 ---
  \node[fadedorgan, text width=6.8in, below=0.38in of s5.south west, anchor=north west] (s6) {
  ...
  % Connecting Arrows
  \draw[activearrow] (s1.east) -- (s2.west);
  \draw[activearrow] (s2.east) -- (s3.west);
  \draw[fadedarrow] (s3.south) -- (s3.south |- s4.north);
  \draw[fadedarrow] (s4.west) -- (s5.east);
  \draw[fadedarrow, dashed] (s5.south) -- (s5.south |- s6.north);
