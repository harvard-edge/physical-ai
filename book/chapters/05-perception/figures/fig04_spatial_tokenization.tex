...
% Input: RGB-D Image
  \node[flowbox, draw=ethblue!70, text width=1.55in] (input) at (0.80in, 0.00in) {
    {\scriptsize\bfseries\color{ethblue} RAW OBSERVATION}\\[3pt]
  ...
  \node[flowbox, draw=ethpetrol!80, text width=1.70in] (vit) at (2.95in, 0.00in) {
    {\scriptsize\bfseries\color{ethpetrol} SPATIAL ENCODER}\\[3pt]
  ...
  \node[flowbox, draw=alertRed!70, fill=alertRed!5, text width=3.80in, minimum height=1.15in] (passive) at (6.70in, 0.00in) {
    {\scriptsize\bfseries\color{alertRed} PASSIVE CLASSIFICATION (Digital ML Paradigm)}\\[3pt]
    {\tiny\color{ethslate}
  ...
  \node[flowbox, draw=safeTeal!80, fill=safeTeal!5, text width=3.80in, minimum height=1.50in] (affordance) at (6.70in, -1.50in) {
    {\scriptsize\bfseries\color{safeTeal} 3D SPATIAL AFFORDANCE TOKENS (Physical AI Paradigm)}\\[3pt]
    {\tiny\color{ethslate}
  ...
  \node[draw=ethdarkblue, fill=ethdarkblue!10, rounded corners=4pt, line width=0.9pt, text width=9.20in, inner sep=5pt, align=center] (downstream) at (4.60in, -3.35in) {
    {\scriptsize\bfseries\color{ethdarkblue} DOWNSTREAM ACTION CONSUMPTION:}\;
