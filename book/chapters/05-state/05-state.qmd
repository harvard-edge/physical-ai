...
6. **Freeze the State and Timing Model (state and timing model):** Author the fifth artifact of the engineering notebook establishing coordinate hierarchies, TTL windows, covariance bounds, and thermal derating regimes.
:::
...
::: {.callout-note title="Cross-Embodiment Anchor: World Modeling and State Persistence Across the 3 Archetypes"}
...
* **Rotation Matrix ($R_B^A \in SO(3)$):** $3 \times 3$ orthonormal rotation matrix satisfying $R^T R = I$ and $\det(R) = +1$.
...
* **Transition Jacobian ($F_t$):** $\left.\frac{\partial f}{\partial \mathbf{x}}\right|_{\hat{\mathbf{x}}_{t-1}}$, representing the Jacobian of the continuous physical transition model.
* **Process Noise Covariance ($Q_t$):** $\mathbb{E}[\mathbf{w}_t \mathbf{w}_t^T]$, capturing unmodeled physical disturbances such as vibration, motor cogging, and turbulence.
...
::: {.callout-decision}
### Notebook checkpoint: State and Timing Model (state and timing model)
Freeze the fifth artifact of the engineering notebook establishing spatial coordinate hierarchies, transform buffer configurations, state TTL lease horizons, uncertainty thresholds, and thermal derating envelopes.
...
: Quantitative Specification Parameters for the State and Timing Model (state and timing model). Coordinate frame configurations, TTL leases, uncertainty bounds, and thermal limits. {#tbl-dossier-state-01}


```yaml
# DOSSIER CHECKPOINT: state and timing model
state_timing_model:
 system_id: "PHYSICAL-AGENT-01"
 version: "1.0.0"
 notebook_checkpoint: "state and timing model"
