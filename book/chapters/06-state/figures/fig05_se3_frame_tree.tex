...
\begin{tikzpicture}[
  ...
  \node[card, draw=ethdarkblue, fill=ethdarkblue!5, text width=7.4in, line width=1.1pt] (title) {
    {\large\bfseries\color{ethdarkblue} DYNAMIC $SE(3)$ COORDINATE FRAME TREE \& TIME-INDEXED BUFFER}\\[3pt]
  ...
  \node[card, draw=ethblue, fill=boxbluebg, text width=3.45in, minimum height=4.3in, below=0.3in of title.south west, anchor=north west] (treecard) {
    {\bfseries\color{ethblue} Coordinate Frame Graph ($T_B^A \in SE(3)$)}\\[6pt]
    ...
    \begin{minipage}{3.3in}
      ...
      \fcolorbox{ethdarkblue}{white}{
        \begin{minipage}{1.7in}
          ...
          \color{ethdarkblue} \texttt{world} (Root)\\
          {\tiny Origin: $[0, 0, 0]^T \in \mathbb{R}^3$}
        \end{minipage}
      }\\[12pt]
      
       {\tiny\color{ethslate}$T_{\text{base}}^{\text{world}}$ (Odometry / Fixed)}\\[6pt]

      % Base Frame: robot_base
      \fcolorbox{ethdarkblue}{white}{
        \begin{minipage}{1.7in}
          \centering\scriptsize\bfseries
          \color{ethdarkblue} \texttt{robot\_base}\\
      ...
      \begin{minipage}{3.2in}
        ...
        \begin{minipage}{1.5in}
          \centering
          {\tiny\color{ethbronze} $T_{\text{cam}}^{\text{base}}$ (Extrinsic)}\\[4pt]
          ...
          {\tiny\color{harvardcrimson} $T_{\text{target}}^{\text{cam}}$ (Vision Token)}\\[4pt]
        ...
        \begin{minipage}{1.5in}
          \centering
          {\tiny\color{ethpetrol} $T_{\text{EE}}^{\text{base}}(q)$ ($1\text{ kHz}$ FK)}\\[4pt]
          ...
          {\tiny\color{ethdarkblue} $T_{\text{tool}}^{\text{EE}}$ (Tool Offset)}\\[4pt]
          \fcolorbox{ethdarkblue}{white}{
            \begin{minipage}{1.35in}
              \centering\scriptsize\bfseries
  ...
  \node[card, draw=ethpurple, fill=boxpurplebg, text width=3.65in, minimum height=4.3in, right=0.3in of treecard] (buffercard) {
    {\bfseries\color{ethpurple} Time-Indexed Buffer \& Lie Tangent Propagation}\\[6pt]
    ...
    \begin{minipage}{3.45in}
      ...
      \fcolorbox{boxamberborder}{white}{
        \begin{minipage}{3.3in}
          ...
          \textbf{\color{ethbronze} 1. Delayed Vision Ingestion ($\Delta t_{\text{lag}} = 65\text{ ms}$):}\\
      ...
      \fcolorbox{boxtealborder}{white}{
        \begin{minipage}{3.3in}
          \raggedright\scriptsize
          \textbf{\color{ethpetrol} 2. Forward Coordinate Chaining ($SE(3)$):}\\
      ...
      \fcolorbox{boxblueborder}{white}{
        \begin{minipage}{3.3in}
          \raggedright\scriptsize
          \textbf{\color{ethdarkblue} 3. Tangent-Space Propagation ($\mathfrak{se}(3)$):}\\
      ...
      \fcolorbox{ethpurple!40}{white}{
        \begin{minipage}{3.3in}
          \raggedright\scriptsize
          \textbf{\color{ethpurple} 4. Real-Time Query Guarantee:}\\
  ...
  \node[card, draw=ethdarkblue, fill=ethdarkblue!5, text width=7.4in, below=0.25in of treecard.south west, anchor=north west] (footer) {
    {\bfseries\color{ethdarkblue} THE SE(3) FRAME CONTRACT: SPATIAL PERSISTENCE UNDER DELAY}\\[2pt]
