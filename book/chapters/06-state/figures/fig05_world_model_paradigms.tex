...
\begin{tikzpicture}[
  ...
  \node[card, draw=ethdarkblue, fill=ethdarkblue!5, text width=7.4in, line width=1.1pt] (header) {
    {\large\bfseries\color{ethdarkblue}\faIcon{database}\;\; THREE PARADIGMS OF PHYSICAL WORLD MODELING}\\[3pt]
    {\footnotesize How should a Physical AI agent represent the environment, predict future dynamics, and survive sensory occlusion?}
  };

  % Column 1: Generative Pixel Diffusion
  \node[card, draw=badredborder, fill=badredbg, text width=2.22in, minimum height=4.3in, below=0.32in of header.south west, anchor=north west] (col1) {
    {\bfseries\color{harvardcrimson}\faIcon{video}\;\; 1. GENERATIVE PIXELS}\\[2pt]
    ...
    \begin{minipage}{2.05in}
      ...
      \centering
      \colorbox{harvardcrimson!15}{\scriptsize\bfseries\color{harvardcrimson}\faIcon{times-circle}\; UNVIABLE FOR REAL-TIME}
    \end{minipage}
  };
  ...
  \node[card, draw=goodtealborder, fill=goodtealbg, text width=2.22in, minimum height=4.3in, right=0.37in of col1] (col2) {
    {\bfseries\color{ethpetrol}\faIcon{brain}\;\; 2. LATENT DYNAMICS}\\[2pt]
    ...
    \begin{minipage}{2.05in}
      ...
      \centering
      \colorbox{ethpetrol!15}{\scriptsize\bfseries\color{ethpetrol}\faIcon{check-circle}\; OPTIMAL FOR INTENT PLANNING}
    \end{minipage}
  };
  ...
  \node[card, draw=blueborder, fill=bluebg, text width=2.22in, minimum height=4.3in, right=0.37in of col2] (col3) {
    {\bfseries\color{ethdarkblue}\faIcon{project-diagram}\;\; 3. METRIC $SE(3)$ TREES}\\[2pt]
    ...
    \begin{minipage}{2.05in}
      ...
      \centering
      \colorbox{ethdarkblue!15}{\scriptsize\bfseries\color{ethdarkblue}\faIcon{shield-alt}\; MANDATORY FOR SAFETY}
    \end{minipage}
  };
  ...
  \node[card, draw=ethpurple, fill=ethpurple!6, text width=7.4in, below=0.25in of col2.south, anchor=north] (footer) {
    {\bfseries\color{ethpurple}\faIcon{layer-group}\;\; THE HYBRID STATE ARCHITECTURE OF PHYSICAL AI}\\[2pt]
