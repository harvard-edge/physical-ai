...
% Input: Untrusted VLM Proposal
  \node[draw=proposalAmber, fill=proposalAmber!10, rounded corners=4pt, line width=1pt, text width=1.50in, minimum height=2.35in, inner sep=7pt, align=center, anchor=north] (input) at (0, 0.00in) {
    {\scriptsize\bfseries\color{proposalAmber} UNTRUSTED PROPOSAL}\\[4pt]
  ...
  \node[gatecard, draw=ethblue!80] (g1) at (2.05in, 0.00in) {
    {\scriptsize\bfseries\color{ethblue} GATE 1}\\[3pt]
  ...
  \node[gatecard, draw=ethpetrol!80] (g2) at (4.30in, 0.00in) {
    {\scriptsize\bfseries\color{ethpetrol} GATE 2}\\[3pt]
  ...
  \node[gatecard, draw=ethpurple!80] (g3) at (6.55in, 0.00in) {
    {\scriptsize\bfseries\color{ethpurple} GATE 3}\\[3pt]
  ...
  \node[draw=safeTeal, fill=safeTeal!10, rounded corners=4pt, line width=1pt, text width=4.15in, inner sep=6pt, anchor=north] (pass) at (2.10in, -2.55in) {
    {\scriptsize\bfseries\color{safeTeal} ALL GATES PASSED: ISSUE INTENT LEASE ($\mathcal{L}_{\text{intent}}$)}\\[2pt]
    {\tiny\color{ethdarkblue}Commit validated 3D target frame and workspace envelope to System 1.5 Trajectory Planner}
  };

  \node[draw=harvardcrimson, fill=harvardcrimson!10, rounded corners=4pt, line width=1pt, text width=4.15in, inner sep=6pt, anchor=north] (fail) at (6.70in, -2.55in) {
    {\scriptsize\bfseries\color{harvardcrimson} GATE FAILED: EMIT STRUCTURED ABSTENTION}\\[2pt]
