...
\begin{tikzpicture}[
  ...
  % Top Title Banner
  \node[draw=ethdarkblue, fill=ethdarkblue!5, rounded corners=5pt, line width=1pt, text width=8.60in, inner sep=7pt, align=center] (title) at (4.30in, 0) {
    {\normalsize\bfseries\color{ethdarkblue}\faIcon{project-diagram}\;\; THE 4-STAGE VLM VISUAL \& SPATIAL AFFORDANCE GROUNDING PIPELINE}\\[2pt]
    {\scriptsize\color{ethslate}Lifting Open-Vocabulary Natural Language to Metric $SE(3)$ Contact Frames on Edge Accelerators}
  };

  % Stage 1: Linguistic Prompt & Visual Attention
  \node[stagecard, draw=ethpurple!80] (st1) at (0, -0.65in) {
    {\scriptsize\bfseries\color{ethpurple}\faIcon{comment-alt}\; STAGE 1}\\[3pt]
  ...
  \node[stagecard, draw=ethblue!80] (st2) at (2.25in, -0.65in) {
    {\scriptsize\bfseries\color{ethblue}\faIcon{crop-alt}\; STAGE 2}\\[3pt]
  ...
  \node[stagecard, draw=ethpetrol!80] (st3) at (4.50in, -0.65in) {
    {\scriptsize\bfseries\color{ethpetrol}\faIcon{cube}\; STAGE 3}\\[3pt]
  ...
  \node[stagecard, draw=proposalAmber!90] (st4) at (6.75in, -0.65in) {
    {\scriptsize\bfseries\color{proposalAmber}\faIcon{hand-paper}\; STAGE 4}\\[3pt]
  ...
  \node[draw=safeTeal!80, fill=safeTeal!8, rounded corners=4pt, line width=0.9pt, text width=8.60in, inner sep=6pt, anchor=north] (summary) at (4.30in, -3.30in) {
    \centering
    {\scriptsize\bfseries\color{safeTeal}\faIcon{check-circle}\;\; OUTPUT CONTRACT:}
