...
\begin{tikzpicture}[
  ...
  \node[draw=ethdarkblue, fill=ethdarkblue!5, rounded corners=5pt, line width=1pt, text width=9.60in, inner sep=7pt, align=center] (title) at (4.80in, 0) {
    {\normalsize\bfseries\color{ethdarkblue}\faIcon{shield-alt}\;\; MINIMAL-INTERVENTION CONTROL BARRIER FUNCTION (CBF) PROJECTION}\\[2pt]
    {\scriptsize\color{ethslate}Projecting Untrusted Neural Action Proposals onto the Forward-Invariant Safe Set $\mathcal{C}$ at 1000 Hz}
  };

  % --- LEFT CARD CONTAINER ---
  \node[draw=cardborder, fill=cardbg, rounded corners=5pt, line width=0.8pt, text width=4.65in, minimum height=5.20in, inner sep=10pt, below=0.15in of title.south west, anchor=north west] (card_left) {};
  ...
  \node[anchor=north west, text width=4.40in, align=left, inner sep=0pt] at ($(card_left.north west)+(0.15in,-0.15in)$) {
    {\small\bfseries\color{ethdarkblue}\faIcon{shapes}\;\; State Space: Forward Invariance $\mathcal{C} = \{x \in \mathbb{R}^n : h(x) \ge 0\}$}\\[3pt]
  ...
  \coordinate (Lcenter) at ($(card_left.north)+(0, -2.75in)$);
  ...
  \node[font=\sffamily\bfseries\scriptsize, text=measuredTeal, rotate=-65] at ($(Lcenter)+(-0.22in,-0.60in)$) {
    $\partial \mathcal{C}: h(x) = 0$
  };

  % Safe Interior Label
  \node[font=\sffamily\bfseries\small, text=measuredTeal] at ($(Lcenter)+(-1.35in,-0.40in)$) {
    \faIcon{check-circle}\; $\text{Int}(\mathcal{C})$\\[1pt]
    {\scriptsize $h(x) > 0$}
  };

  % Unsafe Region Label
  \node[font=\sffamily\bfseries\small, text=consequenceCoral] at ($(Lcenter)+(1.15in,0.70in)$) {
    \faIcon{times-circle}\; $\mathcal{C}^c$ (Unsafe)\\[1pt]
    {\scriptsize $h(x) < 0$}
  };

  % Current State Point x(t)
  \coordinate (x0) at ($(Lcenter)+(-0.45in,-0.25in)$);
  ...
  \node[draw=measuredTeal, fill=white, rounded corners=3pt, font=\sffamily\scriptsize, text=ethdarkblue, inner sep=3.5pt] at ($(Lcenter)+(0,-1.38in)$) {
    \textbf{Forward Invariance Invariant:} $\forall x_0 \in \mathcal{C} \implies x(t) \in \mathcal{C} \quad \forall t \ge 0$
  };


  % --- RIGHT CARD CONTAINER ---
  \node[draw=cardborder, fill=cardbg, rounded corners=5pt, line width=0.8pt, text width=4.65in, minimum height=5.20in, inner sep=10pt, below=0.15in of title.south east, anchor=north east] (card_right) {};
  ...
  \node[anchor=north west, text width=4.40in, align=left, inner sep=0pt] at ($(card_right.north west)+(0.15in,-0.15in)$) {
    {\small\bfseries\color{ethdarkblue}\faIcon{calculator}\;\; Action Space: Minimal-Intervention Quadratic Program}\\[3pt]
  ...
  \coordinate (Rcenter) at ($(card_right.north)+(0, -3.65in)$);

  % Admissible Half-Plane
  \fill[tealBg, rounded corners=4pt] ($(Rcenter)+(-2.15in,-0.80in)$) rectangle ($(Rcenter)+(2.15in,0.35in)$);
  % Inadmissible Half-Plane (Top)
  \fill[coralBg, rounded corners=4pt] ($(Rcenter)+(-2.15in,0.35in)$) rectangle ($(Rcenter)+(2.15in,0.85in)$);

  % Barrier Hyperplane in Action Space: a_cbf^T u = b_cbf
  \draw[line width=1.3pt, measuredTeal] ($(Rcenter)+(-2.15in,0.35in)$) -- ($(Rcenter)+(2.15in,0.35in)$)
    node[pos=0.04, above right=1pt, font=\sffamily\bfseries\tiny, text=measuredTeal] {Admissible Barrier Boundary: $L_g h(x) u = -L_f h(x) - \alpha(h(x))$};

  % Origin in Action Space
  \coordinate (u_orig) at ($(Rcenter)+(-1.20in,-0.25in)$);
  \fill[ethslate] (u_orig) circle (2.5pt) node[below, font=\sffamily\tiny, text=ethslate] {$u = 0$};

  % Candidate Proposal p_t (In Forbidden Half-Plane)
  \coordinate (u_prop) at ($(Rcenter)+(0.55in,0.65in)$);
  ...
  \coordinate (u_proj) at ($(Rcenter)+(0.55in,0.35in)$);
  \fill[measuredTeal] (u_proj) circle (3.5pt)
    node[below=3pt, font=\sffamily\bfseries\scriptsize, text=measuredTeal] {$u_t^* = \Pi_{\mathcal{U}_{\text{safe}}}(p_t)$};
  ...
  % Right Angle Symbol
  \draw[line width=0.6pt, ethslate] ($(u_proj)+(0.10in,0)$) -- ($(u_proj)+(0.10in,0.10in)$) -- ($(u_proj)+(0,0.10in)$);

  % Permitted Region Text
  \node[font=\sffamily\bfseries\scriptsize, text=measuredTeal] at ($(Rcenter)+(-0.90in,0.05in)$) {
    \faIcon{check}\; $\mathcal{U}_{\text{safe}}(x)$ (Permitted Action Space)
  };
  \node[font=\sffamily\bfseries\scriptsize, text=consequenceCoral] at ($(Rcenter)+(-0.90in,0.60in)$) {
    \faIcon{ban}\; Forbidden (Breaches Invariant)
  };

  % Performance Summary Badge
  \node[draw=ethdarkblue!30, fill=white, rounded corners=3pt, font=\sffamily\scriptsize, text=ethdarkblue, inner sep=3.5pt] at ($(Rcenter)+(0,-0.60in)$) {
