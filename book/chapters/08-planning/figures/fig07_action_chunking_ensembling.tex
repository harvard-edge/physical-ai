...
\begin{tikzpicture}[
  ...
  \node[anchor=west, font=\tiny\bfseries, color=proposalamber] at ($(s1_pos) + (0.15in, -0.49in)$) {
    Chunk $A^{(0)}$ ($t=0\text{ ms}$): $H=16$
  };
  ...
  \node[anchor=west, font=\tiny\bfseries, color=authorityviolet] at ($(s1_pos) + (1.75in, -0.83in)$) {
    Chunk $A^{(1)}$ ($80\text{ ms}$): $H=16$
  };
  ...
  \node[anchor=west, font=\tiny\bfseries, color=measuredteal] at ($(s1_pos) + (1.75in, -1.17in)$) {
    Chunk $A^{(2)}$ ($160\text{ ms}$): $H=16$
  };
