...
\begin{tikzpicture}[
  ...
  \node[card, fill=navyfill, draw=structuralnavy, text width=7.5in, align=left] (header) {
    \textbf{\color{structuralnavy}\faIcon{wave-square}\; DERIVATIVE CONTINUITY $\mathcal{C}^2$ VS. DISCONTINUOUS WAYPOINT STEPPING} \hfill {\scriptsize\color{neutralgray} Actuator Protection \& Gearbox Shock Mitigation}
  };
  ...
  \node[font=\footnotesize\bfseries, color=consequencecoral] at ($(col_left) + (1.77in, -0.17in)$) {
    \faIcon{times-circle}\; Raw Chunk Stepping ($\mathcal{C}^0$ Failure $\to$ Hardware Shock)
  };
  ...
  \node[font=\footnotesize\bfseries, color=measuredteal] at ($(col_right) + (1.77in, -0.17in)$) {
    \faIcon{check-circle}\; $\mathcal{C}^2$ Quintic Spline ($\|\dddot{q}\| \le j_{\text{max}}$ Continuous)
  };
  ...
  \node[anchor=north west, font=\tiny, text width=3.35in, color=consequencecoral] at ($(r4_left) + (0.1in, -0.06in)$) {
    \textbf{\faIcon{exclamation-triangle}\; Hardware Destruction:}\\
  ...
  \node[anchor=north west, font=\tiny, text width=3.35in, color=structuralnavy] at ($(r4_right) + (0.1in, -0.06in)$) {
    \textbf{\color{measuredteal}\faIcon{shield-alt}\; Actuator Protection:}\\
