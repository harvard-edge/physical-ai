...
This chapter establishes how embodied systems enforce deterministic, microsecond-level safety around stochastic neural policies:

1. **Section 9.2 (Control Barrier Functions & Forward Invariance):** We formulate continuously differentiable barrier certificates $h(\mathbf{x}) \ge 0$ and prove set invariance via Nagumo's condition.
2. **Section 9.3 (Minimal-Intervention CBF-QPs):** We implement active-set Quadratic Program solvers in static microcontroller SRAM to minimally project candidate neural proposals ($\mathbf{u}_{\text{cand}}$) onto safe half-spaces in under $175\,\mu\text{s}$.
3. **Section 9.4 (Contact Mechanics & Dynamic Impedance):** We formulate virtual spring-damper compliance ($\mathbf{M}_d, \mathbf{D}_d, \mathbf{K}_d$) and passivity energy tanks to stabilize stiff physical contact interactions.
4. **Section 9.5 & 9.6 (Field-Oriented Control & E-Stop Hierarchy):** We execute Clarke/Park transforms for $20\text{ kHz}$ SVPWM inverter switching and establish the IEC 60204-1 emergency stop fallback hierarchy.
5. **Section 9.7 to 9.10 (Incident Autopsy, SAFE-01 Contract & Handoff):** We dissect a flexspline fracture caused by a $140\text{ ms}$ OS writeback freeze, compile the formal `SAFE-01` safety ledger, expose reflex fallacies, and provide the forward handoff to compute placement in Chapter 10.
