...
\begin{tikzpicture}[
  ...
  \node[draw=ethdarkblue, fill=ethdarkblue!5, rounded corners=5pt, line width=1pt, text width=9.60in, inner sep=7pt, align=center] (title) at (4.80in, 0) {
    {\normalsize\bfseries\color{ethdarkblue}\faIcon{stopwatch}\;\; 1000 HZ BARE-METAL MCU TIMING TICK \& ZERO-ALLOCATION BUDGET}\\[2pt]
    {\scriptsize\color{ethslate}Deterministic Microsecond Execution on ARM Cortex-M4 Silicon vs. Linux MPU Scheduler Latency Jitter}
  };

  % --- TOP CARD: The 1000 Hz / 1.0 ms Microcontroller Hard Loop ---
  \node[draw=cardborder, fill=cardbg, rounded corners=5pt, line width=0.8pt, text width=9.60in, minimum height=3.30in, inner sep=10pt, below=0.15in of title.south west, anchor=north west] (card_mcu) {};

  \node[anchor=north west, text width=9.30in, align=left, inner sep=0pt] at ($(card_mcu.north west)+(0.15in,-0.15in)$) {
    {\small\bfseries\color{ethdarkblue}\faIcon{microchip}\;\; System 1 Real-Time Domain: Cortex-M4 Hard Real-Time 1.0 ms ($1000\,\mu\text{s}$) ISR Slot}\\[2pt]
  ...
  \draw[fill=measuredTeal!40, draw=measuredTeal, line width=1.1pt, rounded corners=2pt]
    ...
    node[midway, font=\sffamily\bfseries\tiny, text=ethdarkblue, align=center] {\faIcon{calculator}\; QP Solver\\{\tiny 70 $\mu$s ($L_g h u \ge -\alpha$)}};
  ...
  \draw[fill=white, draw=ethslate!50, dashed, line width=0.8pt, rounded corners=2pt]
    ...
    node[midway, font=\sffamily\bfseries\scriptsize, text=ethslate, align=center] {
      \faIcon{shield-alt}\;\; Deterministic Slack Margin ($825\,\mu\text{s}$ Headroom / $82.5\%$ Idle)\\[1pt]
  ...
  \node[anchor=north west, text width=9.30in, align=left, inner sep=0pt] at ($(card_mpu.north west)+(0.15in,-0.15in)$) {
    {\small\bfseries\color{consequenceCoral}\faIcon{exclamation-triangle}\;\; The Linux MPU Reality (System 2): Non-Deterministic Scheduling Jitter}\\[2pt]
  ...
  \draw[fill=consequenceCoral!30, draw=consequenceCoral, line width=1pt, rounded corners=2pt]
    ...
    node[midway, font=\sffamily\bfseries\tiny, text=consequenceCoral, align=center] {
      \faIcon{skull-crossbones}\; \textbf{$P_{99.9}$ Tail Latency Spike: $140\text{--}240\text{ ms}$}\\[1pt]
  ...
  \node[draw=consequenceCoral, fill=white, rounded corners=3pt, font=\sffamily\scriptsize, text=consequenceCoral, inner sep=4pt, anchor=west]
    at ($(Morigin)+(6.95in, 0.275in)$) {
      \textbf{\faIcon{ban}\; Misses Deadline by $240\times$}
    };
