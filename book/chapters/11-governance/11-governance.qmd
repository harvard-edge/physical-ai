...
1. Architect the four-state human-machine authority engine (`MANUAL`, `SHARED_ADMITTANCE`, `AUTONOMOUS`, `ESTOP`) with strict bare-metal hardware preemption hierarchies.
2. Formulate and enforce $\mathcal{C}^2$ quintic smoothstep blending filters to eliminate actuator torque spikes and gearbox shock loads during sudden human joystick takeovers.
3. Prevent the policy endogeneity trap by applying microsecond-level intervention tagging and truncated episode slicing to continuous data flywheel retraining pipelines.
4. Construct IEEE 1588 PTP hardware-timestamped SHA-256 hash chains linking sensory observations, model weights, and physical actuator outputs for forensic non-repudiation.
5. Architect fail-safe dual-bank A/B firmware rollback schemes that execute automatic sub-millisecond rollbacks if safety loops fail initial boot verification.
6. Deploy independent dual-heartbeat hardware watchdogs across application processors and real-time microcontrollers to intercept software freezes and communications dropouts.
7. Analyze the Flywheel Collapse incident where un-demarcated intervention logs caused three autonomous fleet vehicles to smash into structural columns at $1.8\text{ m/s}$.
8. Author the formal engineering specification establishing authority contracts, blending bounds, telemetry lineage, and bootloader rollback regimes.
...
## Introduction: The Fallacy of Ungoverned Autonomy in Physical Space
...
To protect physical hardware and human operators, the governance runtime implements deterministic bare-metal authority state machines that execute sub-microsecond preemption in silicon (Section 11.2). When human operators seize control, the system filters the setpoint handoff through quintic smoothstep polynomials that guarantee continuous torque derivatives and eliminate mechanical shock loads (Section 11.3). Beyond real-time execution, continuous fleet learning requires overcoming the policy endogeneity trap, where naive retraining on emergency takeover logs induces catastrophic policy collapse (Section 11.4). By deploying microsecond-level hardware intervention tagging, the logging engine partitions telemetry into nominal demonstrations, recovery episodes, and truncated failure logs (Section 11.5). These streams are anchored by IEEE 1588 hardware timestamps and recursive SHA-256 hash chains to establish tamper-evident flight recorders (Section 11.6). We ground these principles in the forensic autopsy of the Flywheel Collapse incident (Section 11.7) and formalize the complete governance specification (Section 11.8) before exposing common runtime pitfalls (Section 11.9) and advancing to whole-system verification (Section 11.10).
...
* **Dual-Heartbeat Watchdogs:** The Linux host MPU and the real-time MCU exchange bidirectional $100\text{ Hz}$ ($10.0\text{ ms}$) heartbeats across isolated hardware timer channels. If the host application processor hangs due to a memory compaction stall, kernel page fault, or neural runtime panic, the MCU monitors the watchdog hierarchy: two consecutive missed packets ($\tau_{\text{soft}} = 20.0\text{ ms}$) initiate a Category 1 graceful trajectory deceleration, while an un-refreshed hardware windowed watchdog ($\tau_{\text{hw}} = 35.0\text{ ms}$) trips an unmaskable interrupt that severs Safe Torque Off lines. If the MCU watchdog expires, hardware reset lines assert STO immediately. During over-the-air firmware updates, an isolated $50.0\text{ ms}$ qualification window triggers automatic hardware rollback if the newly flashed image fails to initialize real-time safety loops.
...
High-capacity policy representations (such as Diffusion Policies and Action Chunking Transformers) excel at memorizing multi-timestep temporal conditioning prefixes. The model does not learn to avoid pillars; it learns that *charging directly at concrete pillars at maximum speed is the necessary precondition for initiating a turn*.
...
If optical camera glare, dust accumulation, or neural inference jitter delays the perception of the $1.2\text{ m}$ visual trigger by just $\Delta t_{\text{jitter}} = 40\text{ ms}$, the vehicle travels an additional:

$$\Delta d = v \cdot \Delta t_{\text{jitter}} = 1.8\text{ m/s} \times 0.040\text{ s} = 0.072\text{ m} \quad (7.2\text{ cm})$$

Because the human emergency swerve was performed at the physical friction limit ($\mu g$), losing $7.2\text{ cm}$ of clearance exhausts the vehicle's lateral turning envelope. The vehicle crashes into the structural pillar at full speed ($1.8\text{ m/s}$), producing fleet-wide destruction.
...
While standard behavioral cloning suffers from quadratic error compounding $\mathcal{O}(T^2 \epsilon_{\text{class}})$ over task horizon $T$ due to unmodeled covariate shift, DAgger data aggregation guarantees a linear regret bound $\mathcal{O}(T \epsilon_{\text{class}})$. Complete mathematical proofs of these regret bounds and covariate shift dynamics are cataloged in Appendix C.12.
...
This recursive architecture enforces three critical properties:

1. **Mathematical Tamper Evidence:** Because each frame incorporates the digest of its predecessor, any retroactive alteration to historical logs (such as clearing an intervention flag or modifying recorded camera frames) breaks the downstream hash chain. A cryptographic audit over $N$ frames detects modifications in $\mathcal{O}(N)$ time (or $\mathcal{O}(\log N)$ using hierarchical Merkle audit trees as proven in Appendix C.12).
...
3. **Deterministic DMA Telemetry Isolation:** Logging a $1000\text{ Hz}$ telemetry stream produces approximately $1\text{ MB/s}$ of structured binary data. To prevent flash storage writes from introducing interrupt latency into high-frequency motor control loops, telemetry frames are assembled in double-buffered static SRAM ($512\text{ KB}$) and transferred to dedicated non-volatile storage partitions (such as UFS 3.1 or NVMe) via a zero-copy Direct Memory Access (DMA) controller. The cryptographic hashing is offloaded to a hardware cryptographic engine, executing in under $2\,\mu\text{s}$ and leaving real-time processor cores entirely free for $1\text{ kHz}$ control execution.

To guarantee firmware integrity across continuous fleet deployments, the storage subsystem implements a **Dual-Bank A/B Flash Architecture**. The active runtime executes from Bank A (Golden Image) while over-the-air updates are written to Bank B. The bootloader cryptographically verifies the update image using asymmetric Ed25519 digital signatures against an immutable hardware root of trust. Upon reboot, an isolated hardware watchdog enforces a $50.0\text{ ms}$ qualification window. If the new firmware fails to initialize real-time safety loops or assert motor watchdogs within $50.0\text{ ms}$, the bootloader executes an automatic hardware rollback to Bank A, preventing bricked or un-governed states in the field.
...
* **System:** Fleet of Autonomous Heavy Pallet Lifters ($m = 1{,}200\text{ kg}$, $v = 1.8\text{ m/s}$, front-wheel steering, dual-camera vision stack).
* **Incident:** Three autonomous fleet vehicles collided head-on with reinforced concrete columns at $1.8\text{ m/s}$, delivering $1{,}944\text{ J}$ of kinetic energy each ($5{,}832\text{ J}$ cumulative impact), shearing steering kingpins, rupturing hydraulic mast lines, and damaging building columns ($\$240{,}000\text{ USD}$ total damage).
* **Telemetry Snapshot:** Un-demarcated logging recorded $10{,}000\text{ hours}$ of driving; pillar approach at $T - 2.500\text{ s}$; manual joystick takeover at $T - 1.200\text{ s}$ ($d = 1.2\text{ m}$, $\dot{\psi} = 0.8\text{ rad/s}$); `is_human_intervention` flag omitted in user-space daemon; Model v4.2 retrained and deployed; sensor dust delayed visual trigger by $40\text{ ms}$ ($7.2\text{ cm}$ forward displacement); impact at $T_{\text{impact}}$.
* **Root Cause:** Behavioral cloning diffusion policies fitted spurious causal correlations between high-speed pillar approaches and last-second evasive turns, training the policy to actively accelerate toward columns to trigger turning maneuvers.
* **Architectural Flaw:** Absence of hardware-level microsecond intervention tagging ($\mathbf{1}_{\text{human\_active}}$), lack of DAgger episode truncation, and failure to enforce bare-metal MCU dynamic stopping envelopes.
* **Provenance:** Engineered composite drawn from documented commercial autonomous mobile robot fleet incidents; telemetry constructed for teaching.
:::
...
* **The Four-Rung Qualification Ladder:** Progressing candidate neural policies systematically from Historical Telemetry Replay to Software-in-the-Loop (SIL), Hardware-in-the-Loop (HIL) on Target Silicon, and Shadow Fleet Deployment.
