...
* **Dual-Heartbeat Watchdogs:** The Linux host MPU and the real-time MCU exchange bidirectional $20\text{ ms}$ heartbeats across isolated hardware timer channels. If the host application processor hangs due to a memory compaction stall, kernel page fault, or neural runtime panic, the MCU detects the missing heartbeat within $20\text{ ms}$ and executes a deterministic Category 1 or Category 2 fallback. If the MCU watchdog expires, hardware reset lines assert Safe Torque Off immediately.
