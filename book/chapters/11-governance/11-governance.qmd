...
To guarantee deterministic human-machine authority, enforce smooth transition dynamics, secure telemetry provenance, and protect firmware integrity, the governance subsystem formalizes the **Governance Record Specification (GOV-01)** (@tbl-dossier-gov-01).
...
: The Human-Authority and Governance Record Specification (**GOV-01**). Quantitative authority parameters, bumpless blending bounds, cryptographic lineage standards, and dual-bank bootloader invariants bridging Governance to Verification. {#tbl-dossier-gov-01}
...
::: {.callout-fallacy title="Passive Ingestion of Raw Human Interventions Automatically Improves End-to-End Autonomous Policies (The Passive Flywheel Myth)"}
**The Fallacy:** *“Because web-scale foundation models scale monotonically with the volume of passively scraped user interaction logs, autonomous robotic fleets can achieve self-improving super-human performance simply by continuously uploading all operational telemetry—including human safety-driver interventions—into cloud behavioral cloning retraining pipelines.”*

**The Reality:** In digital software platforms, user interactions are passive and exogenous: a user clicking a search link does not alter the underlying laws of the web server. In physical AI, the closed loop is fundamentally endogenous: the robot's commanded actions directly dictate its future physical states ($s_t \to a_t \to s_{t+1}$).

When a human safety driver seizes manual controls to avert a crash, the pre-intervention trajectory represents an out-of-distribution failure mode driven by policy drift ($s_{\text{danger}}$), while the human intervention represents a corrective maneuver ($a_{\text{swerve}}$). Ingesting these sequences without microsecond demarcation flags creates a fatal spurious correlation in the behavioral cloning loss:

$$\mathcal{L}_{\text{BC}}(\theta) = \mathbb{E}_{(s_t, a_t) \sim \mathcal{D}} \left[ \left\| \pi_\theta(s_t) - a_t \right\|_2^2 \right]$$

The neural network learns that entering an extreme, near-collision state is the necessary conditional trigger for initiating an evasive turn. When deployed to the fleet, the policy actively steers toward obstacles at high speed. Passive data flywheels inevitably collapse physical policies. Embodied learning requires active **DAgger-principled dataset slicing**: truncating the pre-intervention approach as a negative failure episode ($\mathcal{D}_{\text{failure}}$) and extracting only the post-takeover recovery trajectory as a supervised corrective demonstration ($\mathcal{D}_{\text{recovery\_dagger}}$).
:::

::: {.callout-fallacy title="Software-Based Emergency Stop Handlers in User-Space Operating Systems Are Sufficient for Functional Safety"}
**The Fallacy:** *“Registering a software signal handler (`SIGINT`/`SIGTERM`), a ROS 2 subscriber callback, or a high-level Python exception handler to intercept Emergency Stop button events is sufficient to satisfy functional safety requirements in autonomous physical systems.”*

**The Reality:** General-purpose operating systems (including desktop Linux and standard POSIX distributions) and user-space managed runtimes are inherently non-deterministic. User-space signal delivery can be delayed by hundreds of milliseconds or blocked indefinitely due to memory allocator heap locks (`malloc` mutex contention), kernel virtual memory page faults, thread priority inversions, unhandled kernel panics, or Out-Of-Memory (OOM) killer freezes.

If a human operator hits an E-Stop button while a user-space process is stalled on a dynamic memory allocation, the robot will continue driving under its last latched motor command. 

Functional safety standards (such as **ISO 13849-1 Category 4 / PL e** and **IEC 62061 SIL 3**) strictly forbid relying on complex software stacks for primary safety shutoff. Primary E-Stop interlocks must be implemented through hardwired, optocoupler-isolated **Safe Torque Off (STO)** lines that directly de-energize the gate driver power rails of the motor inverters, physically preventing current flow through the power MOSFET/IGBT bridges. In software, safety supervision must reside exclusively within bare-metal, dual-heartbeat microcontroller routines running in lockstep silicon with static zero-allocation SRAM buffers, executing safety preemption within $<5.0\,\mu\text{s}$.
:::

::: {.callout-caution title="Instantaneous Control Authority Switching Causing Mechanical Jerk and Inverter Overcurrent Tripping"}
**The Hazard:** Switching control authority between an autonomous neural policy and a human operator as an instantaneous discrete step change ($\Delta \mathbf{u} = \mathbf{u}_{\text{human}} - \mathbf{u}_{\text{policy}}$) within a single control cycle ($1.0\text{ ms}$).

**The Physical Failure:** An instantaneous step discontinuity in commanded control setpoints implies an infinite rate of change of torque ($\dot{\boldsymbol{\tau}} \to \infty$) and infinite commanded mechanical jerk ($\dddot{\mathbf{q}} \to \infty$). 

In electrical actuators, demanding an instantaneous torque step forces the field-oriented controller (FOC) to drive a step increase in quadrature-axis current ($i_q$). The rapid current derivative ($di/dt \to \infty$) induces massive inductive flyback voltage spikes ($V = L \frac{di}{dt}$) across motor phase windings, driving gate driver drain-source voltages ($V_{DS}$) into desaturation. On-chip hardware comparators instantly trip overcurrent and desaturation fault registers ($I_{\text{trip}}$), shutting down the inverter bridge and leaving the mechanism unpowered mid-maneuver.

Simultaneously, the mechanical shock load injects violent stress waves through the drivetrain, causing severe backlash collisions in planetary or cycloidal gearboxes, stripping precision gear teeth, and shearing structural keyway pins.

**The Defensive Practice:** Enforce $\mathcal{C}^2$ quintic smoothstep bumpless transfer filtering ($S(\alpha) = 6\alpha^5 - 15\alpha^4 + 10\alpha^3$) across a calibrated blending window ($T_{\text{blend}} = 150\text{ ms}$). Because $\dot{S}(0)=\dot{S}(1)=0$ and $\ddot{S}(0)=\ddot{S}(1)=0$, both torque derivatives and accelerations remain continuous across the handoff boundary, strictly bounding mechanical jerk ($\|\dddot{\mathbf{q}}\| \le 25.0\text{ rad/s}^3$) and limiting torque derivative rates ($\|\dot{\boldsymbol{\tau}}\| \le 150.0\text{ N}\cdot\text{m/s}$).
:::

::: {.callout-caution title="Synchronous Blocking Flash Disk Writes in Real-Time Telemetry Logging (Interrupt Starvation)"}
**The Hazard:** Executing synchronous, blocking file write calls (such as standard POSIX `write()`, `fsync()`, or blocking SPI NOR / eMMC / NVMe flash writes) directly within high-frequency telemetry logging loops or real-time control threads.

**The Physical Failure:** Non-volatile NAND/NOR flash memory architectures require physical erase-before-write operations managed by internal flash translation layers (FTLs). While single-page write operations require $1\text{--}5\text{ ms}$, periodic flash block erase cycles, wear-leveling re-allocations, and garbage collection passes routinely block the storage bus for $10\text{--}100\text{ ms}$.

When a real-time control thread ($1000\text{ Hz}$, $1.0\text{ ms}$ period) or an interrupt service routine (ISR) invokes a synchronous disk write, the CPU pipeline stalls awaiting flash bus acknowledgment. The processor misses successive timer interrupts, starves PWM commutation loops of updated setpoints, and fails to service hardware watchdogs. The safety microcontroller detects a software hang, asserts a watchdog reset, and trips a sudden Category 0 emergency stop, dropping loads or causing high-speed vehicle skids.

**The Defensive Practice:** Decouple telemetry collection from non-volatile storage using zero-allocation, lock-free ring buffers residing in dedicated static on-chip SRAM ($512\text{ KB}$ DTCM / AXI SRAM). Use an independent Direct Memory Access (DMA) controller to stream committed ring buffer frames asynchronously to non-volatile storage during bus idle cycles, or offload telemetry serialization entirely to a dedicated I/O coprocessor core, guaranteeing that real-time $1000\text{ Hz}$ actuator control loops execute with zero interrupt latency and zero storage I/O jitter.
:::
...
- **The Release Dossier (`RELEASE-01`):** Formulating the definitive engineering gatekeeper contract to certify whether an embodied AI system is qualified to deploy, requires conditional staging, or must be refused physical release.
