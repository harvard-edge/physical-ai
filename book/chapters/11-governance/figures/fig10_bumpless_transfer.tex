...
\begin{tikzpicture}[
  ...
  \node[draw=navy!30, fill=slatelight, rounded corners=4pt, line width=0.8pt, text width=7.2in, align=center, inner sep=6pt] (topheader) at (3.5in, 0.4in) {
    \textbf{\faIcon{wave-square}\; HARD STEP TAKEOVER VS. $\mathcal{C}^2$ BUMPLESS TORQUE BLENDING}\\[2pt]
    {\footnotesize Takeover event at $t = 100\,\text{ms}$ with blending transition window $\tau_{\text{blend}} = 100\,\text{ms}$}
  };

  % =========================================================================
  % PANEL 1: COMMANDED TORQUE / TRAJECTORY u(t)
  % =========================================================================
  \coordinate (p1_origin) at (0, -2.0in);
  
  \node[anchor=south west, font=\sffamily\small\bfseries, text=navy] at ($(p1_origin)+(-0.4in, 1.55in)$) {
    \faIcon{chart-line}\; (a) Commanded Control Torque $u(t)$ [Nm]
  };
  ...
  \node[font=\sffamily\scriptsize\bfseries, text=coral, fill=corallight, draw=coral!50, rounded corners=2pt, inner sep=2pt] 
    at ($(p1_origin)+(2.95in, 1.18in)$) {\faIcon{bolt}\; Step Switch ($\Delta u = +0.7\,\text{Nm}$)};
  ...
  \node[font=\sffamily\scriptsize\bfseries, text=teal, fill=teallight, draw=teal!50, rounded corners=2pt, inner sep=2pt] 
    at ($(p1_origin)+(4.8in, 0.65in)$) {\faIcon{check-circle}\; $\mathcal{C}^2$ Smoothstep Blending};

  % =========================================================================
  % PANEL 2: TORQUE DERIVATIVE / MOTOR JERK \dot{u}(t)
  % =========================================================================
  \coordinate (p2_origin) at (0, -4.2in);
  
  \node[anchor=south west, font=\sffamily\small\bfseries, text=navy] at ($(p2_origin)+(-0.4in, 1.55in)$) {
    \faIcon{tachometer-alt}\; (b) Torque Rate of Change $\dot{u}(t) \propto \dddot{q}(t)$ (Actuator Jerk) [Nm/s]
  };
  ...
  % =========================================================================
  \coordinate (p3_origin) at (0, -6.4in);
  
  \node[anchor=south west, font=\sffamily\small\bfseries, text=navy] at ($(p3_origin)+(-0.4in, 1.55in)$) {
    \faIcon{bolt}\; (c) Actuator Phase Current $I(t)$ [A] \& Gearbox Backlash Stress
  };
  ...
  \node[font=\sffamily\tiny\bfseries, text=coral, fill=corallight, draw=coral!60, rounded corners=2pt, inner sep=2pt, align=left]
    at ($(p3_origin)+(3.0in, 1.20in)$) {\textbf{Overcurrent Fault Spike (7.4 A)}\\\faIcon{exclamation-circle} Backlash shudder \& tooth wear};
