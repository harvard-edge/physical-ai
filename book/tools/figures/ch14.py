...
def gen_ch14_handshake_fsm():
    ...
    # -------------------------------------------------------------
    # TOP SECTION: 4-PHASE PROTOCOL PIPELINE (Y: 62 to 182)
    # -------------------------------------------------------------
    top_y = 62
    top_h = 122
    ...
    phases = [
        ...
        ("PHASE 2 · INTERCEPT &amp; AUTH", "Hardware Verification", [
            ...
            "HMAC-SHA256 &amp; Nonce valid",
            "Freshness: Δt_transit &le; 100 ms"
        ], PURPLE, 252),
        ...
        ("PHASE 4 · CONFIRM &amp; ACTIVE", "Active Authority", [
            ...
            "Active lease (T_lease &le; 100 ms)"
        ], TEAL, 688)
    ]
    ...
    for idx, (p_tag, p_sub, p_items, col, px) in enumerate(phases):
        ...
        svg.append(f'<rect x="{px}" y="{py_card}" width="{pw}" height="20" rx="6" fill="{col}" fill-opacity="0.12"/>')
        svg.append(f'<text x="{px+10}" y="{py_card+14}" font-size="8.5" font-weight="700" fill="{col}">{p_tag}</text>')
        svg.append(f'<text x="{px+10}" y="{py_card+33}" font-size="9" font-weight="700" fill="{INK}">{p_sub}</text>')
        for l_idx, l_txt in enumerate(p_items):
            svg.append(f'<text x="{px+10}" y="{py_card+47+l_idx*12}" font-size="7.5" fill="{SLATE}">• {l_txt}</text>')
    ...
    # -------------------------------------------------------------
    # BOTTOM LEFT: 5 DISCRETE STATES & TRANSITION FSM (X: 25 to 575, Y: 196 to 495)
    # -------------------------------------------------------------
    fsm_x = 25
    fsm_y = 196
    fsm_w = 550
    ...
    svg.append(f'<text x="{fsm_x+14}" y="{fsm_y+16}" font-size="9.5" font-weight="700" fill="{NAVY}">DISCRETE AUTHORITY STATE MACHINE (S_t &isin; &#123;Auto, Pending, Blend, Manual, Fallback&#125;)</text>')

    # State S_auto
    s1_x, s1_y, s1_w, s1_h = fsm_x + 14, fsm_y + 36, 158, 88
    ...
    svg.append(f'<text x="{s1_x+8}" y="{s1_y+44}" font-size="7.5" fill="{SLATE}">• High-level action chunks</text>')
    svg.append(f'<text x="{s1_x+8}" y="{s1_y+58}" font-size="7.5" fill="{SLATE}">• 1 kHz Enforcer bounds</text>')
    svg.append(f'<text x="{s1_x+8}" y="{s1_y+72}" font-size="7.5" fill="{SLATE}">• Background state tracking</text>')

    # State S_pending
    s2_x, s2_y, s2_w, s2_h = fsm_x + 196, fsm_y + 36, 158, 88
    ...
    svg.append(f'<text x="{s2_x+8}" y="{s2_y+30}" font-size="8" font-weight="600" fill="{INK}">Authority: Frozen / Check</text>')
    svg.append(f'<text x="{s2_x+8}" y="{s2_y+44}" font-size="7.5" fill="{SLATE}">• Handshake timer armed</text>')
    svg.append(f'<text x="{s2_x+8}" y="{s2_y+58}" font-size="7.5" fill="{SLATE}">• Token &amp; nonce validation</text>')
    svg.append(f'<text x="{s2_x+8}" y="{s2_y+72}" font-size="7.5" fill="{CORAL}">• Guard: t &le; T_timeout (500ms)</text>')

    # State S_blend
    s3_x, s3_y, s3_w, s3_h = fsm_x + 378, fsm_y + 36, 158, 88
    ...
    svg.append(f'<text x="{s3_x+8}" y="{s3_y+13}" font-size="8.5" font-weight="700" fill="{BRONZE}">S_blend · BUMPLESS RAMP</text>')
    ...
    svg.append(f'<text x="{s3_x+8}" y="{s3_y+58}" font-size="7.5" fill="{SLATE}">• Window τ_blend &ge; 1.5|Δu|/j_max</text>')
    svg.append(f'<text x="{s3_x+8}" y="{s3_y+72}" font-size="7.5" fill="{SLATE}">• Jerk bounded: j &le; j_allowable</text>')

    # State S_fallback (Bottom Left)
    s5_x, s5_y, s5_w, s5_h = fsm_x + 14, fsm_y + 185, 230, 96
    ...
    svg.append(f'<text x="{s5_x+8}" y="{s5_y+44}" font-size="7.5" fill="{SLATE}">• Deterministic braking: a_brake = μ·g (6.87 m/s²)</text>')
    svg.append(f'<text x="{s5_x+8}" y="{s5_y+58}" font-size="7.5" fill="{SLATE}">• Monotonic lock: No upward transitions during motion</text>')
    svg.append(f'<text x="{s5_x+8}" y="{s5_y+72}" font-size="7.5" fill="{SLATE}">• Forensic buffer freeze: 10s pre/post trigger window</text>')
    svg.append(f'<text x="{s5_x+8}" y="{s5_y+86}" font-size="7.5" fill="{TEAL}">• Exit: Zero velocity (v = 0) + Signed Re-init</text>')

    # State S_manual (Bottom Right)
    s4_x, s4_y, s4_w, s4_h = fsm_x + 306, fsm_y + 185, 230, 96
    ...
    svg.append(f'<text x="{s4_x+8}" y="{s4_y+44}" font-size="7.5" fill="{SLATE}">• Direct steering / torque dispatch</text>')
    svg.append(f'<text x="{s4_x+8}" y="{s4_y+58}" font-size="7.5" fill="{SLATE}">• Continuous deadman lease (T_lease &le; 100 ms)</text>')
    svg.append(f'<text x="{s4_x+8}" y="{s4_y+72}" font-size="7.5" fill="{PETROL}">• Subordinate to Tier-1 Safety Barrier h(x) &ge; 0</text>')
    svg.append(f'<text x="{s4_x+8}" y="{s4_y+86}" font-size="7.5" fill="{MUTED}">• Shadow logging of background policy inferences</text>')

    # TRANSITION ARROWS & LABELS
    # S_auto -> S_pending
    svg.append(f'<line x1="{s1_x+s1_w}" y1="{s1_y+30}" x2="{s2_x}" y2="{s2_y+30}" stroke="{AMBER}" stroke-width="1.5" marker-end="url(#arr-bronze)"/>')
    svg.append(f'<text x="{(s1_x+s1_w+s2_x)/2}" y="{s1_y+24}" font-size="7" font-weight="700" fill="{AMBER}" text-anchor="middle">Override Trigger</text>')

    # S_pending -> S_blend
    svg.append(f'<line x1="{s2_x+s2_w}" y1="{s2_y+30}" x2="{s3_x}" y2="{s3_y+30}" stroke="{BRONZE}" stroke-width="1.5" marker-end="url(#arr-bronze)"/>')
    svg.append(f'<text x="{(s2_x+s2_w+s3_x)/2}" y="{s2_y+24}" font-size="7" font-weight="700" fill="{BRONZE}" text-anchor="middle">Auth Verified</text>')

    # S_blend -> S_manual
    svg.append(f'<line x1="{s3_x+s3_w/2}" y1="{s3_y+s3_h}" x2="{s4_x+s4_w/2+50}" y2="{s4_y}" stroke="{TEAL}" stroke-width="1.5" marker-end="url(#arr-teal)"/>')
    svg.append(f'<text x="{s3_x+s3_w/2+15}" y="{s4_y-20}" font-size="7" font-weight="700" fill="{TEAL}">Ramp Complete (α = 1.0)</text>')

    # S_pending -> S_fallback (Timeout drop)
    svg.append(f'<line x1="{s2_x+s2_w/2}" y1="{s2_y+s2_h}" x2="{s5_x+s5_w/2+30}" y2="{s5_y}" stroke="{CORAL}" stroke-width="1.5" stroke-dasharray="3,3" marker-end="url(#arr-coral)"/>')
    svg.append(f'<rect x="{s2_x-20}" y="{s2_y+s2_h+16}" width="140" height="15" rx="3" fill="{BG_WHITE}" stroke="{CORAL}" stroke-width="0.8"/>')
    svg.append(f'<text x="{s2_x+50}" y="{s2_y+s2_h+27}" font-size="7" font-weight="700" fill="{CORAL}" text-anchor="middle">Timeout t &gt; T_timeout (500 ms)</text>')

    # S_manual -> S_fallback (Lease Loss / Inattention)
    svg.append(f'<line x1="{s4_x}" y1="{s4_y+40}" x2="{s5_x+s5_w}" y2="{s5_y+40}" stroke="{CORAL}" stroke-width="1.5" stroke-dasharray="3,3" marker-end="url(#arr-coral)"/>')
    svg.append(f'<text x="{(s4_x+s5_x+s5_w)/2}" y="{s4_y+34}" font-size="7" font-weight="700" fill="{CORAL}" text-anchor="middle">Lease Expired / Touch Loss (&gt;100 ms)</text>')

    # S_fallback -> S_auto (Safe Re-init Reset)
    svg.append(f'<path d="M {s5_x+40} {s5_y} C {s5_x+40} {s1_y+s1_h+25}, {s1_x+40} {s1_y+s1_h+25}, {s1_x+40} {s1_y+s1_h}" fill="none" stroke="{BLUE}" stroke-width="1.4" marker-end="url(#arr-blue)"/>')
    svg.append(f'<text x="{s1_x+40}" y="{s1_y+s1_h+20}" font-size="7" font-weight="700" fill="{BLUE}" text-anchor="middle">v = 0 &amp; Reset</text>')

    # -------------------------------------------------------------
    # BOTTOM RIGHT: 3-TIER ARBITRATION STACK & EVIDENCE SCHEMA (X: 588 to 895, Y: 196 to 495)
    # -------------------------------------------------------------
    arb_x = 588
    arb_y = 196
    arb_w = 307
    ...
    tiers = [
        ("TIER 1 · ABSOLUTE VETO (MCU REFLEX)", "1000 Hz Hard Real-Time Enforcer", [
            "CBF Invariant: h(x) &ge; 0 · Over-current trip",
            "Kinematic floor · Rejects unsafe human commands"
        ], PETROL),
        ("TIER 2 · SUPERVISORY OVERRIDE", "Authenticated Human Input (α = 1.0)", [
            "Bounded deadman lease (T_lease &le; 100 ms)",
    ...
    for t_num, t_title, t_desc, col in tiers:
        svg.append(f'<rect x="{arb_x+10}" y="{ty}" width="{arb_w-20}" height="48" rx="5" fill="{col}" fill-opacity="0.05" stroke="{col}" stroke-width="1"/>')
        svg.append(f'<rect x="{arb_x+10}" y="{ty}" width="4" height="48" rx="2" fill="{col}"/>')
        svg.append(f'<text x="{arb_x+20}" y="{ty+13}" font-size="8" font-weight="700" fill="{col}">{t_num}</text>')
        svg.append(f'<text x="{arb_x+20}" y="{ty+26}" font-size="7.5" font-weight="600" fill="{INK}">{t_title}</text>')
    ...
    svg.append(f'<text x="{arb_x+20}" y="{ey+28}" font-size="7.5" font-family="monospace" fill="{NAVY}">⟨t_PTP, S_auth, x_state, u_prop, u_enf, u_act, ID_op⟩</text>')
    ...
    for idx, sp in enumerate(schema_points):
        svg.append(f'<text x="{arb_x+20}" y="{ey+42+idx*12}" font-size="7.2" fill="{SLATE}">{sp}</text>')
...
def gen_ch14_bumpless_transfer_dynamics():
    ...
    svg.append(f'<text x="{W/2}" y="44" class="subtitle">Eliminating Mechanical Jerk Shock, Drive Slip, and Integrator Windup via Dynamic Blending τ_blend &ge; 1.5|Δu| / j_allowable</text>')

    # -------------------------------------------------------------
    # LEFT PANEL: UNBLENDED STEP HANDOVER (X: 25 to 455, Y: 60 to 485)
    # -------------------------------------------------------------
    lx = 25
    ly = 60
    lw = 425
    ...
    # Path: 0.0 before t0, instant jump to 1.0 at t0
    ...
    svg.append(f'<text x="{t0_x+10}" y="{y_one1+12}" font-size="7.5" font-weight="700" fill="{AMBER}">Step: 0.0 ⟶ 1.0 (1 clock cycle)</text>')
    ...
    svg.append(f'<text x="{t0_x+10}" y="{y_target2-14}" font-size="7.5" font-weight="700" fill="{CORAL}">Δu = 25 N·m Step + Severe Ringing</text>')
    ...
    svg.append(f'<text x="{t0_x+12}" y="{y_spike3+12}" font-size="7.5" font-weight="700" fill="{CORAL}">⚡ j_max &gt; 6000 m/s³ &gt;&gt; j_allowable</text>')
    svg.append(f'<text x="{t0_x+12}" y="{y_spike3+24}" font-size="7" fill="#FCA5A5">Driveline shock &amp; gear pin fracture</text>')
    ...
    # Slip trajectory breaking traction
    ...
    svg.append(f'<text x="{t0_x+12}" y="{ty4+18}" font-size="7.5" font-weight="700" fill="{CRIMSON}">Traction Loss: Wheels Lock / Skid</text>')

    # Bottom summary for Left Panel
    svg.append(f'<rect x="{lx+12}" y="{ly+lh-36}" width="{lw-24}" height="26" rx="4" fill="{CORAL}" fill-opacity="0.08" stroke="{CORAL}" stroke-width="0.8"/>')
    svg.append(f'<text x="{lx+lw/2}" y="{ly+lh-20}" font-size="8" font-weight="700" fill="{CORAL}" text-anchor="middle">FAILURE: Infinite Jerk · Mechanical Gearbox Shock · Stopping Path +3.67 m</text>')

    # -------------------------------------------------------------
    # RIGHT PANEL: C¹ CUBIC BUMPLESS BLENDING (X: 470 to 900, Y: 60 to 485)
    # -------------------------------------------------------------
    rx = 470
    ry = 60
    rw = 425
    ...
    # Smooth cubic S-curve: 3(t/tau)^2 - 2(t/tau)^3
    cubic_alpha = (
    ...
    svg.append(f'<text x="{rt0_x+25}" y="{y_one1+12}" font-size="7.5" font-weight="700" fill="{TEAL}">C¹ Cubic: α(t) = 3(t/τ)² - 2(t/τ)³</text>')
    ...
    svg.append(f'<text x="{rt0_x+20}" y="{y_target2-12}" font-size="7.5" font-weight="700" fill="{TEAL}">Smooth Sigmoidal Torque Transition (Zero Ringing)</text>')
    ...
    # Parabolic bell: peak jerk at midpoint rt0_x + (rt1_x - rt0_x)/2
    r_mid_x = (rt0_x + rt1_x) / 2
    y_bell_peak = ty3 + 24 # Bounded jerk peak
    ...
    svg.append(f'<text x="{r_mid_x}" y="{y_bell_peak+15}" font-size="7.5" font-weight="700" fill="{TEAL}" text-anchor="middle">j_max = 1.5|Δu|/τ_blend &le; 12.0 m/s³ ✓</text>')
    ...
    svg.append(f'<text x="{rt0_x+25}" y="{ty4+18}" font-size="7.5" font-weight="700" fill="{TEAL}">Linear Adhesion Maintained (s &le; 0.04 &lt;&lt; s_crit)</text>')

    # Bottom summary for Right Panel
    svg.append(f'<rect x="{rx+12}" y="{ry+rh-36}" width="{rw-24}" height="26" rx="4" fill="{TEAL}" fill-opacity="0.08" stroke="{TEAL}" stroke-width="0.8"/>')
    svg.append(f'<text x="{rx+rw/2}" y="{ry+rh-20}" font-size="8" font-weight="700" fill="{TEAL}" text-anchor="middle">SUCCESS: Zero Shock · Continuous Acceleration · Integrators Pre-Biased to x(t)</text>')
